...
#if WPA_ENABLE == 0
int newpower_time_out_current_state;
int wpa_debug_level;
#endif

#define DECLARE_SSID_BUF(var) char var[IEEE80211_MAX_SSID_LEN * 4 + 1]
#define DEBUG_PARMA_SDIO INSERT_PARMA_INFO
...
//	union iwreq_data wrqu;
	...
	if (/*priv->fwrelease < 0x09000000 这里就不进行固件版本判断了，8686 8385 应该是相同的*/1) {
		switch (status_code) {
		case 0x00:
			break;
		case 0x01:
			 ...
			 u16 command)
{
	...
	if (assoc_req->secinfo.WPAenabled || assoc_req->secinfo.WPA2enabled) {
		...
		memcpy(priv->__mac_addr,priv->cur_bss->bssid,ETH_ALEN);

		/*初始化WPA相关部分，并设通过WPA程序生成可用的IEs*/
		DEBUG_PARMA_SDIO;
		assoc_req->wpa_ie_len = priv->__wpa_ie_len;
		memcpy(assoc_req->wpa_ie,priv->__wpa_ie,priv->__wpa_ie_len);//下面是经过调用的IELEN
		...
		lbs_deb_hex(LBS_DEB_JOIN, "ASSOC_CMD: WPA IE",(u8 *)assoc_req->wpa_ie,assoc_req->wpa_ie_len);
...
/*这个函数添加的目的是，将一个搜索到的AP，而且匹配合格后的信息，加入到PRIV中，以备关联时用*/
static int set_bssinfo_to_priv(struct lbs_private *priv,
				 struct bss_descriptor *bss)
{
#if 0 //这里不用这样做，因为后面的过程中，如果找到了当前所要关联的AP，那么将BSS存储到cur_bss中去
	priv->__wpa_ie_len = bss->wpa_ie_len;
	memcpy(priv->__wpa_ie,bss->wpa_ie, priv->__wpa_ie_len);
#endif
	return 0;
	//
}
...
static int xxx_find_ssid(struct lbs_private *priv , struct assoc_request * assoc , char *ssid , char *key)
{
	...
	list_for_each_entry_bssdes(iter_bss, &priv->network_list, list) {
		...
		if (strcmp(iter_bss->ssid,ssid) == 0)
		{
			...
			//如果找到了一个SSID相同的,那么就检查他的加密类型然后对其进行设置
			if (iter_bss->wpa_ie[0] == WLAN_EID_GENERIC) //wpa
			{
				assoc->secinfo.wep_enabled=0;
				assoc->secinfo.WPAenabled=1;


				assoc->flags|=1<<ASSOC_FLAG_WPA_IE;
				assoc->flags|=1<<ASSOC_FLAG_WPA_MCAST_KEY;
				assoc->flags|=1<<ASSOC_FLAG_SECINFO;
				//
			}

			if (iter_bss->rsn_ie[0] == WLAN_EID_RSN) //wpa2
			{
				assoc->secinfo.wep_enabled=0;
			...
			if (assoc->secinfo.WPAenabled || assoc->secinfo.WPA2enabled)
			{
				...
				// 这里就是计算真正的WPA密码算法，让其设置到关联中，将密码设置进去
			...
			{
				extern int newpower_time_out_current_state;
				newpower_time_out_current_state = 8; // 由于这里不是WPA加密因此不进行判断就直接进入连接状态
			}
...
{	
	...
	printk("please input wep/wpa key:%s\n",key);
