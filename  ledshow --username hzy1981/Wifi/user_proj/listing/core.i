...
#line 3 "..\\sdio\\src\\core.c"
#line 4 "..\\sdio\\src\\core.c"
...
#line 6 "..\\sdio\\src\\core.c"
#line 7 "..\\sdio\\src\\core.c"
#line 8 "..\\sdio\\src\\core.c"
#line 9 "..\\sdio\\src\\core.c"
#line 10 "..\\sdio\\src\\core.c"
#line 11 "..\\sdio\\src\\core.c"
#line 12 "..\\sdio\\src\\core.c"
#line 13 "..\\sdio\\src\\core.c"
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (err && cmd->retries && ((host)->caps & (1 << 4))) {
		if (cmd->resp[0] & (1 << 2))
			cmd->retries = 0;
	}
...
void mmc_set_data_timeout(struct mmc_data *data, const struct mmc_card *card)
{
	...
	if (((card)->type == 1)) {
		...
		if (timeout_us > limit_us || ((card)->state & (1<<3))) {
...
static void mmc_power_up(struct mmc_host *host)
{
	...
	if(0){
		host->ios.chip_select = 1;
		host->ios.bus_mode = 2;
	} else {
		host->ios.chip_select = 0;
		host->ios.bus_mode = 1;
	}
	host->ios.power_mode = 1;
...
struct mmc_card *mmc_alloc_card(struct mmc_host *host)
{
	static struct mmc_card marvel_gcard;
...
int mmc_add_card(struct mmc_card *card)
{
	...
	if (((card->host)->caps & (1 << 4))) {
		printf( "%s: new %s%s card on SPI\n",
			("wifi"),
			((card)->state & (1<<2)) ? "high speed " : "",
			type);
	} else {
		printf( "%s: new %s%s card at address %04x\n",
			("wifi"),
			((card)->state & (1<<2)) ? "high speed " : "",
			type, card->rca);
	}
...
struct mmc_host *mmc_alloc_host(void)
{
	...
	host->max_seg_size = (1<<12);

	host->max_req_size = (1<<12);
	host->max_blk_size = 512;
	host->max_blk_count = (1<<12) / 512;
...
int sdio_enable_func(struct sdio_func *func)
{
	...
	ret = mmc_io_rw_direct(func->card, 0, 0, 0x02, 0, &reg);
	if (ret)
		goto err;

	reg |= 1 << func->num;

	ret = mmc_io_rw_direct(func->card, 1, 0, 0x02, reg, 0);
	if (ret)
		goto err;
...
int sdio_claim_irq(struct sdio_func *func, sdio_irq_handler_t *handler)
{
	...
	ret = mmc_io_rw_direct(func->card, 0, 0, 0x04, 0, &reg);
	if (ret)
		return ret;
	...
	ret = mmc_io_rw_direct(func->card, 1, 0, 0x04, reg, 0);
	if (ret)
		return ret;
...
{
	if (id->class != ( u8)(~0) && id->class != func->class)
		return 0;
	if (id->vendor != (u16)(~0) && id->vendor != func->vendor)
		return 0;
	if (id->device != (u16)(~0) && id->device != func->device)
		return 0;
	return id;
}

const struct sdio_device_id if_sdio_ids[3]={
	(~0),0x02df,0x9103,
	(~0),0x02df,0x9104,
	0,0,0
};
...
int sdio_set_block_size(struct sdio_func *func, unsigned blksz)
{
	...
	if (blksz == 0) {
		blksz = (((func->max_blksize)<(func->card->host->max_blk_size))? (func->max_blksize):(func->card->host->max_blk_size));
		blksz = (((blksz)<(512u))? (blksz):(512u));
	}
...
struct lbs_private *sdio_bus_probe(struct sdio_func *func)
{
	...
	if (!id){
		printf("Cann't find support modules!\n");
