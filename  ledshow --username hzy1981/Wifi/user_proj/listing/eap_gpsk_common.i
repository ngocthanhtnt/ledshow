#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 21 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
#line 22 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
static int eap_gpsk_gkdf_cmac(const unsigned char *psk  ,
			      const unsigned char *data  , size_t data_len,
			      unsigned char *buf, size_t len  )
{
	unsigned char *opos;
	size_t i, n, hashlen, left, clen;
	unsigned char ibuf[2], hash[16];
	...
	for (i = 1; i <= n; i++) {
		do { (ibuf)[0] = ((unsigned short) (i)) >> 8; (ibuf)[1] = ((unsigned short) (i)) & 0xff; } while (0);
...
#line 111 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"


static int eap_gpsk_derive_keys_helper(unsigned int csuite_specifier,
				       unsigned char *kdf_out, size_t kdf_out_len,
				       ...
				       unsigned char *pk, size_t pk_len)
{
	unsigned char mk[32], *pos, *data;
	size_t data_len, mk_len;
	int (*gkdf)(const unsigned char *_psk, const unsigned char *_data, size_t _data_len,
		    unsigned char *buf, size_t len);
	...
	case 0x000001:
		...
		mk_len = 16;
		break;
#line 138 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
	default:
		return -1;
	}
	...
	if (data == 0)
		return -1;
	pos = data;
	do { (pos)[0] = ((unsigned short) (psk_len)) >> 8; (pos)[1] = ((unsigned short) (psk_len)) & 0xff; } while (0);
	pos += 2;
	memcpy(pos, psk, psk_len);
	pos += psk_len;
	do { (pos)[0] = (unsigned char) ((((unsigned int) (0x00000000)) >> 24) & 0xff); (pos)[1] = (unsigned char) ((((unsigned int) (0x00000000)) >> 16) & 0xff); (pos)[2] = (unsigned char) ((((unsigned int) (0x00000000)) >> 8) & 0xff); (pos)[3] = (unsigned char) (((unsigned int) (0x00000000)) & 0xff); } while (0);  
	pos += 4;
	do { (pos)[0] = ((unsigned short) (csuite_specifier)) >> 8; (pos)[1] = ((unsigned short) (csuite_specifier)) & 0xff; } while (0);  
	pos += 2;
	memcpy(pos, seed, seed_len);  
	wpa_hexdump_key(0, "EAP-GPSK: Data to MK derivation",
			data, data_len);
	...
	os_free(data);
	wpa_hexdump_key(0, "EAP-GPSK: MK", mk, mk_len);

	if (gkdf(mk, seed, seed_len, kdf_out, kdf_out_len) < 0)
		return -1;

	pos = kdf_out;
	wpa_hexdump_key(0, "EAP-GPSK: MSK", pos, 64);
	memcpy(msk, pos, 64);
	pos += 64;

	wpa_hexdump_key(0, "EAP-GPSK: EMSK", pos, 64);
	memcpy(emsk, pos, 64);
	pos += 64;

	wpa_hexdump_key(0, "EAP-GPSK: SK", pos, sk_len);
	memcpy(sk, pos, sk_len);
	pos += sk_len;

	if (pk) {
		wpa_hexdump_key(0, "EAP-GPSK: PK", pos, pk_len);
		memcpy(pk, pos, pk_len);
	}
...
static int eap_gpsk_derive_keys_aes(const unsigned char *psk, size_t psk_len,
				    const unsigned char *seed, size_t seed_len,
				    unsigned char *msk, unsigned char *emsk, unsigned char *sk, size_t *sk_len,
				    unsigned char *pk, size_t *pk_len)
{
	...
	unsigned char kdf_out[64 + 64 + 16 +
		   16];
	...
	*pk_len = 16;
	...
	return eap_gpsk_derive_keys_helper(0x000001,
...
#line 262 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
...
int eap_gpsk_derive_keys(const unsigned char *psk, size_t psk_len, int vendor,
			 int specifier,
			 const unsigned char *rand_peer, const unsigned char *rand_server,
			 const unsigned char *id_peer, size_t id_peer_len,
			 const unsigned char *id_server, size_t id_server_len,
			 unsigned char *msk, unsigned char *emsk, unsigned char *sk, size_t *sk_len,
			 unsigned char *pk, size_t *pk_len)
{
	unsigned char *seed, *pos;
	size_t seed_len;
	int ret;

	wpa_printf(0, "EAP-GPSK: Deriving keys (%d:%d)",
		   vendor, specifier);

	if (vendor != 0x00000000)
		return -1;

	wpa_hexdump_key(0, "EAP-GPSK: PSK", psk, psk_len);

	 
	seed_len = 2 * 32 + id_server_len + id_peer_len;
	seed = os_malloc(seed_len);
	if (seed == 0) {
		wpa_printf(0, "EAP-GPSK: Failed to allocate memory "
			   "for key derivation");
		return -1;
	}

	pos = seed;
	memcpy(pos, rand_peer, 32);
	pos += 32;
	memcpy(pos, id_peer, id_peer_len);
	pos += id_peer_len;
	memcpy(pos, rand_server, 32);
	pos += 32;
	memcpy(pos, id_server, id_server_len);
	pos += id_server_len;
	wpa_hexdump(0, "EAP-GPSK: Seed", seed, seed_len);
	...
	case 0x000001:
		ret = eap_gpsk_derive_keys_aes(psk, psk_len, seed, seed_len,
					       ...
					       pk, pk_len);
		break;
#line 336 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
	default:
		wpa_printf(0, "EAP-GPSK: Unknown cipher %d:%d used in "
...
static int eap_gpsk_compute_mic_aes(const unsigned char *sk, size_t sk_len,
				    const unsigned char *data, size_t len, unsigned char *mic)
{
	if (sk_len != 16) {
		wpa_printf(0, "EAP-GPSK: Invalid SK length %lu for "
			 ...
			 int specifier, const unsigned char *data, size_t len, unsigned char *mic)
{
	...
	case 0x000001:
		...
		break;
#line 415 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_gpsk_common.c"
	default:
		wpa_printf(0, "EAP-GPSK: Unknown cipher %d:%d used in "
