...
#line 3 "..\\sdio\\src\\sdio.c"
#line 4 "..\\sdio\\src\\sdio.c"
...
#line 6 "..\\sdio\\src\\sdio.c"
#line 7 "..\\sdio\\src\\sdio.c"
#line 8 "..\\sdio\\src\\sdio.c"
#line 9 "..\\sdio\\src\\sdio.c"
...
int mmc_send_io_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
	...
	if (rocr)
		*rocr = cmd.resp[((host)->caps & (1 << 4)) ? 1 : 0];
...
int mmc_send_if_cond(struct mmc_host *host, u32 ocr)
{
	...
	cmd.flags = ((1 << 7)|(1 << 9)) | ((1 << 0)|(1 << 2)|(1 << 4)) | (3 << 5);
...
static int sdio_read_cccr(struct mmc_card *card)
{
	...
	ret = mmc_io_rw_direct(card, 0, 0, 0x00, 0, &data);
	if (ret)
		goto out;
	...
	ret = mmc_io_rw_direct(card, 0, 0, 0x08, 0, &data);
	if (ret)
		goto out;
	...
	if (cccr_vsn >= 1) {
		ret = mmc_io_rw_direct(card, 0, 0, 0x12, 0, &data);
		if (ret)
			goto out;
	...
	if (cccr_vsn >= 2) {
		ret = mmc_io_rw_direct(card, 0, 0, 0x13, 0, &data);
		if (ret)
			goto out;
...
static int sdio_disable_cd(struct mmc_card *card)
{
	...
	ret = mmc_io_rw_direct(card, 0, 0, 0x07, 0, &ctrl);
	if (ret)
		return ret;

	ctrl |= 0x80;

	return mmc_io_rw_direct(card, 1, 0, 0x07, ctrl, 0);
}
...
static int sdio_enable_hs(struct mmc_card *card)
{
	...
	if (!(card->host->caps & (1 << 2)))
		return 0;

	if (!card->cccr.high_speed)
		return 0;

	ret = mmc_io_rw_direct(card, 0, 0, 0x13, 0, &speed);
	if (ret)
		return ret;

	speed |= 0x02;

	ret = mmc_io_rw_direct(card, 1, 0, 0x13, speed, 0);
	if (ret)
		return ret;

	((card)->state |= (1<<2));
	mmc_set_timing(card->host, 2);
...
static int sdio_enable_wide(struct mmc_card *card)
{
	...
	ret = mmc_io_rw_direct(card, 1, 0, 0x07, ctrl, 0);
	if (ret)
		return ret;
...
int mmc_attach_sdio(struct mmc_host *host, u32 ocr)
{
	...
	if (ocr & 0x7F) {
		printf( "%s: card claims to support voltages "
	...
	err = mmc_sdio_init_card(host, host->ocr, 0);
	if (err)
		goto err;
	card = host->card;
...
err:
	printf( "%s: error %d whilst initialising SDIO card\n",
		("wifi"), err);
	return err;
}
...
static int process_sdio_pending_irqs(struct mmc_card *card)
{
	...
	struct sdio_func *func;
	ret = mmc_io_rw_direct(card, 0, 0, 0x05, 0, &pending);
	if (ret) {
...
void sdio_writeb(struct sdio_func *func, u8 b, unsigned int addr, int *err_ret)
{
	...
	ret = mmc_io_rw_direct(func->card, 1, func->num, addr, b, 0);
	if (err_ret)
		*err_ret = ret;
}
...
{
	unsigned mval =(((func->card->host->max_seg_size)<(func->card->host->max_blk_size))? (func->card->host->max_seg_size):(func->card->host->max_blk_size));

	mval = (((mval)<(func->max_blksize))? (mval):(func->max_blksize));
	return (((mval)<(512u))? (mval):(512u));  
}
	...
	unsigned addr, int incr_addr, u8 *buf, unsigned blocks, unsigned blksz)
{
	...
	cmd.flags = ((1 << 7)|(1 << 8)) | ((1 << 0)|(1 << 2)|(1 << 4)) | (1 << 5);

	data.blksz = blksz;
	data.blocks = blocks;
	data.flags = write ? (1 << 8) : (1 << 9);
	...
	unsigned addr, int incr_addr, u8 *buf, unsigned size)
{
	...
	if (func->card->cccr.multi_block && (size > sdio_max_byte_size(func))) {
		...
		max_blocks = (((func->card->host->max_blk_count)<(func->card->host->max_seg_size / func->cur_blksize))? (func->card->host->max_blk_count):(func->card->host->max_seg_size / func->cur_blksize));

		max_blocks = (((max_blocks)<(511))? (max_blocks):(511));
	...
	while (remainder > 0) {
		u16 tmp;
		size = (((remainder)<(sdio_max_byte_size(func)))? (remainder):(sdio_max_byte_size(func)));
		tmp=align_power2(size);	
		;
