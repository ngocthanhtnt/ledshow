#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
...
#line 21 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.h"
...
#line 40 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.h"

#line 48 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.h"
...
struct eap_sake_parse_attr {
	const unsigned char *rand_s;
	const unsigned char *rand_p;
	const unsigned char *mic_s;
	const unsigned char *mic_p;
	const unsigned char *serverid;
	size_t serverid_len;
	const unsigned char *peerid;
	size_t peerid_len;
	const unsigned char *spi_s;
	size_t spi_s_len;
	const unsigned char *spi_p;
	size_t spi_p_len;
	const unsigned char *any_id_req;
	const unsigned char *perm_id_req;
	const unsigned char *encr_data;
	size_t encr_data_len;
	const unsigned char *iv;
	size_t iv_len;
	const unsigned char *next_tmpid;
	size_t next_tmpid_len;
	const unsigned char *msk_life;
};

int eap_sake_parse_attributes(const unsigned char *buf, size_t len,
			      struct eap_sake_parse_attr *attr);
void eap_sake_derive_keys(const unsigned char *root_secret_a, const unsigned char *root_secret_b,
			  const unsigned char *rand_s, const unsigned char *rand_p,
			  unsigned char *tek, unsigned char *msk, unsigned char *emsk);
int eap_sake_compute_mic(const unsigned char *tek_auth,
			 const unsigned char *rand_s, const unsigned char *rand_p,
			 const unsigned char *serverid, size_t serverid_len,
			 const unsigned char *peerid, size_t peerid_len,
			 int peer, const unsigned char *eap, size_t eap_len,
			 const unsigned char *mic_pos, unsigned char *mic);
void eap_sake_add_attr(struct wpabuf *buf, unsigned char type, const unsigned char *data,
		       size_t len);

#line 22 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_sake_common.c"
				   ...
				   const unsigned char *pos)
{
	...
	switch (pos[0]) {
	case 1:
		wpa_printf(0, "EAP-SAKE: Parse: AT_RAND_S");
		if (pos[1] != 2 + 16) {
			wpa_printf(0, "EAP-SAKE: AT_RAND_S with "
		...
		break;
	case 2:
		wpa_printf(0, "EAP-SAKE: Parse: AT_RAND_P");
		if (pos[1] != 2 + 16) {
			wpa_printf(0, "EAP-SAKE: AT_RAND_P with "
				   "invalid length %d", pos[1]);
			return -1;
		}
		attr->rand_p = pos + 2;
		break;
	case 3:
		wpa_printf(0, "EAP-SAKE: Parse: AT_MIC_S");
		if (pos[1] != 2 + 16) {
			wpa_printf(0, "EAP-SAKE: AT_MIC_S with "
				   "invalid length %d", pos[1]);
			return -1;
		}
		attr->mic_s = pos + 2;
		break;
	case 4:
		wpa_printf(0, "EAP-SAKE: Parse: AT_MIC_P");
		if (pos[1] != 2 + 16) {
			wpa_printf(0, "EAP-SAKE: AT_MIC_P with "
				   "invalid length %d", pos[1]);
			return -1;
		}
		attr->mic_p = pos + 2;
		break;
	case 5:
		wpa_printf(0, "EAP-SAKE: Parse: AT_SERVERID");
		attr->serverid = pos + 2;
		attr->serverid_len = pos[1] - 2;
		break;
	case 6:
		wpa_printf(0, "EAP-SAKE: Parse: AT_PEERID");
		attr->peerid = pos + 2;
		attr->peerid_len = pos[1] - 2;
		break;
	case 7:
		wpa_printf(0, "EAP-SAKE: Parse: AT_SPI_S");
		attr->spi_s = pos + 2;
		attr->spi_s_len = pos[1] - 2;
		break;
	case 8:
		wpa_printf(0, "EAP-SAKE: Parse: AT_SPI_P");
		attr->spi_p = pos + 2;
		attr->spi_p_len = pos[1] - 2;
		break;
	case 9:
		wpa_printf(0, "EAP-SAKE: Parse: AT_ANY_ID_REQ");
		if (pos[1] != 4) {
			wpa_printf(0, "EAP-SAKE: Invalid AT_ANY_ID_REQ"
				   " length %d", pos[1]);
			return -1;
		}
		...
		break;
	case 10:
		wpa_printf(0, "EAP-SAKE: Parse: AT_PERM_ID_REQ");
		if (pos[1] != 4) {
			wpa_printf(0, "EAP-SAKE: Invalid "
		...
		break;
	case 128:
		wpa_printf(0, "EAP-SAKE: Parse: AT_ENCR_DATA");
		attr->encr_data = pos + 2;
		attr->encr_data_len = pos[1] - 2;
		break;
	case 129:
		wpa_printf(0, "EAP-SAKE: Parse: AT_IV");
		attr->iv = pos + 2;
		attr->iv_len = pos[1] - 2;
		break;
	case 130:
		wpa_printf(0, "EAP-SAKE: Parse: AT_PADDING");
		for (i = 2; i < pos[1]; i++) {
			if (pos[i]) {
				wpa_printf(0, "EAP-SAKE: AT_PADDING "
		...
		break;
	case 131:
		wpa_printf(0, "EAP-SAKE: Parse: AT_NEXT_TMPID");
	...
	default:
		if (pos[0] < 128) {
			wpa_printf(0, "EAP-SAKE: Unknown non-skippable"
				   " attribute %d", pos[0]);
			return -1;
		}
		wpa_printf(0, "EAP-SAKE: Ignoring unknown skippable "
			   "attribute %d", pos[0]);
		break;
	}

	if (attr->iv && !attr->encr_data) {
		wpa_printf(0, "EAP-SAKE: AT_IV included without "
			   "AT_ENCR_DATA");
		return -1;
	}
...
int eap_sake_parse_attributes(const unsigned char *buf, size_t len,
			      struct eap_sake_parse_attr *attr)
{
	const unsigned char *pos = buf, *end = buf + len;

	os_memset(attr, 0, sizeof(*attr));
	while (pos < end) {
		if (end - pos < 2) {
			wpa_printf(0, "EAP-SAKE: Too short attribute");
			return -1;
		}

		if (pos[1] < 2) {
			wpa_printf(0, "EAP-SAKE: Invalid attribute "
				   "length (%d)", pos[1]);
			return -1;
		}

		if (pos + pos[1] > end) {
			wpa_printf(0, "EAP-SAKE: Attribute underflow");
			return -1;
		}
...
static void eap_sake_kdf(const unsigned char *key, size_t key_len, const char *label,
...
void eap_sake_derive_keys(const unsigned char *root_secret_a, const unsigned char *root_secret_b,
			  const unsigned char *rand_s, const unsigned char *rand_p, unsigned char *tek, unsigned char *msk,
			  unsigned char *emsk)
{
	unsigned char sms_a[16];
	unsigned char sms_b[16];
	unsigned char key_buf[64 + 64];

	wpa_printf(0, "EAP-SAKE: Deriving keys");

	wpa_hexdump_key(0, "EAP-SAKE: Root-Secret-A",
			root_secret_a, 16);
	eap_sake_kdf(root_secret_a, 16,
		     "SAKE Master Secret A",
		     rand_p, 16, rand_s, 16,
		     sms_a, 16);
	wpa_hexdump_key(0, "EAP-SAKE: SMS-A", sms_a, 16);
	eap_sake_kdf(sms_a, 16, "Transient EAP Key",
		     rand_s, 16, rand_p, 16,
		     tek, (16 + 16));
	wpa_hexdump_key(0, "EAP-SAKE: TEK-Auth",
			tek, 16);
	wpa_hexdump_key(0, "EAP-SAKE: TEK-Cipher",
			tek + 16, 16);

	wpa_hexdump_key(0, "EAP-SAKE: Root-Secret-B",
			root_secret_b, 16);
	eap_sake_kdf(root_secret_b, 16,
		     "SAKE Master Secret B",
		     rand_p, 16, rand_s, 16,
		     sms_b, 16);
	wpa_hexdump_key(0, "EAP-SAKE: SMS-B", sms_b, 16);
	eap_sake_kdf(sms_b, 16, "Master Session Key",
		     rand_s, 16, rand_p, 16,
		     key_buf, sizeof(key_buf));
	os_memcpy(msk, key_buf, 64);
	os_memcpy(emsk, key_buf + 64, 64);
	wpa_hexdump_key(0, "EAP-SAKE: MSK", msk, 64);
	wpa_hexdump_key(0, "EAP-SAKE: EMSK", emsk, 64);
}
			 ...
			 const unsigned char *mic_pos, unsigned char *mic)
{
	...
	if (peer) {
		...
		*pos++ = 0x00;
		...
		os_memcpy(_rand, rand_s, 16);
		os_memcpy(_rand + 16, rand_p,
			  16);
	} else {
		if (serverid) {
			os_memcpy(pos, serverid, serverid_len);
		...
		*pos++ = 0x00;
		...
		os_memcpy(_rand, rand_p, 16);
		os_memcpy(_rand + 16, rand_s,
			  16);
	}

	os_memcpy(pos, eap, eap_len);
	os_memset(pos + (mic_pos - eap), 0, 16);

	eap_sake_kdf(tek_auth, 16,
		     peer ? "Peer MIC" : "Server MIC",
		     _rand, 2 * 16, tmp, tmplen,
		     mic, 16);
