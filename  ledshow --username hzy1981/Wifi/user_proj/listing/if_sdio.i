#line 1 "..\\marvel\\driver\\src\\if_sdio.c"
...
#line 2 "..\\marvel\\driver\\src\\if_sdio.c"
...
#line 3 "..\\marvel\\driver\\src\\if_sdio.c"
#line 4 "..\\marvel\\driver\\src\\if_sdio.c"
#line 5 "..\\marvel\\driver\\src\\if_sdio.c"
#line 6 "..\\marvel\\driver\\src\\if_sdio.c"
#line 7 "..\\marvel\\driver\\src\\if_sdio.c"
#line 8 "..\\marvel\\driver\\src\\if_sdio.c"
#line 9 "..\\marvel\\driver\\src\\if_sdio.c"
#line 10 "..\\marvel\\driver\\src\\if_sdio.c"
#line 11 "..\\marvel\\driver\\src\\if_sdio.c"
#line 12 "..\\marvel\\driver\\src\\if_sdio.c"
...
#line 13 "..\\marvel\\driver\\src\\if_sdio.c"
#line 1 "..\\marvel\\driver\\inc\\marvel_main.h"



#line 5 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 6 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 7 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 8 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 9 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 10 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 11 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 12 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 13 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 14 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 15 "..\\marvel\\driver\\inc\\marvel_main.h"
#line 16 "..\\marvel\\driver\\inc\\marvel_main.h"
...
#line 14 "..\\marvel\\driver\\src\\if_sdio.c"
#line 15 "..\\marvel\\driver\\src\\if_sdio.c"
...
#line 16 "..\\marvel\\driver\\src\\if_sdio.c"
...
#line 17 "..\\marvel\\driver\\src\\if_sdio.c"
...
static u16 if_sdio_read_rx_len(struct if_sdio_card *card, int *err)
{
	...
	case 0x10:
	default:  
		rx_len = sdio_readb(card->func, 0x42, &ret);
		if (!ret)
			rx_len <<= card->rx_unit;
		else
			rx_len = 0xffff;	 
		...
		u8 *buffer, unsigned int size)
{
	int ret;
	;
	if (size>(1514 +sizeof(struct rxpd)+8 + 36) ) {
		;
		...
		u8 *buffer, unsigned size)
{
	...
	if (card->model == 0x04) {
		event = sdio_readb(card->func, 0x80fc, &ret);
		if (ret)
			goto out;
...
static int if_sdio_card_to_host(struct if_sdio_card *card)
{
	...
	timeout = 1000;
	while (1) {
		status = sdio_readb(card->func, 0x20, &ret);
		if (ret)
			goto out;
		if (status & 0x08)
			break;
		if (time_after(0, &timeout)) {
			ret = -60;
			goto out;
		}
		mdelay(1);
...
out:
	if (ret)
		printf("problem fetching packet from firmware\n");
...
static void if_sdio_host_to_card_worker(struct if_sdio_card *card)
{
	...
	while (1) {
		...
		ret = sdio_io_rw_ext_helper(card->func, 1, card->ioport, 0, packet->buffer, packet->nb);
...
int if_sdio_send_data(struct lbs_private *priv, u8 *buf, u16 nb)
{
	...
	ret = sdio_io_rw_ext_helper(card->func, 1, card->ioport, 0, buf, size);
 ...
 void if_sdio_interrupt(struct sdio_func *func)
{
	...
	cause = sdio_readb(card->func, 0x05, &ret);
	...
	sdio_writeb(card->func, ~cause, 0x05, &ret);
	if (ret)
		goto out;
...
static int if_sdio_prog_helper(struct if_sdio_card *card)
{
	...
	while (size) {
		...
		chunk_size = (((size)<((size_t)60))? (size):((size_t)60));
		*((__le32*)chunk_buffer) = (chunk_size);
		memcpy(chunk_buffer + 4, firmware, chunk_size);

		;
		ret = sdio_io_rw_ext_helper(card->func, 1, card->ioport, 0, chunk_buffer, 64);
	...
	while (1) {
		u16 req_size;
		req_size = sdio_readb(card->func, 0x10, &ret);
		if (ret)
			goto release;

		req_size |= sdio_readb(card->func, 0x10 + 1, &ret) << 8;
		if (ret)
		{
			goto release;
		}
...
release:
	...
	;
	if (ret)
		printf("failed to load helper firmware\n");
...
static int if_sdio_prog_real(struct if_sdio_card *card)
{
	...
	while (size) {
		...
		while (req_size) {
			chunk_size = (((req_size)<((size_t)512))? (req_size):((size_t)512));

			memcpy(chunk_buffer, firmware, chunk_size);
			;

			ret = sdio_io_rw_ext_helper(card->func, 1, card->ioport, 0, chunk_buffer, ((((chunk_size) + ((32) - 1)) / (32)) * (32)));
	...
	while (1) {
		...
		if (scratch == 0xfedc){
			printf("program firmware success\n");
			break;
		}
...
static int if_sdio_prog_firmware(struct if_sdio_card *card)
{
	...
	if (ret){
		printf("download help firmware failed!\n");
		goto out;
	}
	ret = if_sdio_prog_real(card);
	if (ret){
		printf("download marvel firmware failed!\n");
		goto out;
	}
success:
...
struct lbs_private *if_sdio_probe(struct sdio_func *func,struct sdio_device_id *id)
{
	...
	for (i = 0;i < func->card->num_info;i++) {
		;
		if (strcmp(func->card->info[i],"ID: 04") ==0){
			model = 0x04;
			break; 
		}
	...
	if (i == func->card->num_info) {
		printf("unable to identify card model\n");
	...
	switch (card->model) {
	case 0x04:
		card->scratch_reg = 0x80fe;
		break;
	case 0x0b:
		card->scratch_reg = 0x34;
		break;
	case 0x10:
	default:  
		card->scratch_reg = 0x40;
		break;
	}
	...
	card->ioport = sdio_readb(func, 0x00, &ret);
	if (ret)
		goto release_int;

	card->ioport |= sdio_readb(func, 0x00 + 1, &ret) << 8;
	if (ret)
		goto release_int;

	card->ioport |= sdio_readb(func, 0x00 + 2, &ret) << 16;
	if (ret)
		goto release_int;
	func->if_card=card;
	;
	...
	if (scratch == 0xfedc) 
 	...
 	sdio_writeb(func, 0x0f, 0x04, &ret);
	 if (ret)
	 	goto reclaim;
	;
...
out:
	...
	if(ret)
		...
		return (struct lbs_private *)(priv);
...
u16 wireless_card_rx(u8 *uiprxbuf)
{
	...
	if(ret<0){
			printf("read interrupt error!\n");
			do {if(0) break;}while(1);
	}
	else if(ret&(0x01|0x02)){
...
int pool_sdio_interrupt(struct sdio_func *func)
{
	int ret;
	u8 cause;
	cause=sdio_readb(func, 0x05, &ret);
	if (ret)
		return -5;
	return cause;
