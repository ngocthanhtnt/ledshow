#line 1 "..\\marvel\\driver\\src\\assoc.c"
...
#line 2 "..\\marvel\\driver\\src\\assoc.c"
...
#line 3 "..\\marvel\\driver\\src\\assoc.c"
...
#line 4 "..\\marvel\\driver\\src\\assoc.c"
...
#line 5 "..\\marvel\\driver\\src\\assoc.c"
...
#line 6 "..\\marvel\\driver\\src\\assoc.c"
#line 7 "..\\marvel\\driver\\src\\assoc.c"
#line 8 "..\\marvel\\driver\\src\\assoc.c"
#line 9 "..\\marvel\\driver\\src\\assoc.c"
#line 10 "..\\marvel\\driver\\src\\assoc.c"
...
#line 11 "..\\marvel\\driver\\src\\assoc.c"
...
#line 12 "..\\marvel\\driver\\src\\assoc.c"
...
#line 13 "..\\marvel\\driver\\src\\assoc.c"
...
#pragma pack(2)
static const u8 bssid_any[6]  __attribute__ ((aligned (2))) =
	{ 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
static const u8 bssid_off[6]  __attribute__ ((aligned (2))) =
	...
	u16 *rates_size)
{
	...
	intersection_size = (((*rates_size)<((sizeof(intersection) / sizeof((intersection)[0]))))? (*rates_size):((sizeof(intersection) / sizeof((intersection)[0]))));

	 
	for (i = 0; i < (sizeof(lbs_bg_rates) / sizeof((lbs_bg_rates)[0])) && lbs_bg_rates[i]; i++) {
...
static u8 iw_auth_to_ieee_auth(u8 auth)
{
	if (auth == 0x00000001)
		return 0x00;
	else if (auth == 0x00000002)
		return 0x01;
	else if (auth == 0x00000004)
		return 0x80;

	printf("%s: invalid auth alg 0x%X\n", __func__, auth);
	return 0;
}
...
static int lbs_set_authentication(struct lbs_private *priv, u8 bssid[6], u8 auth)
{
	...
	printf("AUTH_CMD: BSSID %pM, auth 0x%x\n", bssid, cmd.authtype);

	ret = lbs_cmd_with_response(priv, 0x0011, &cmd);
			  ...
			  struct cmd_ds_802_11_associate_response *resp)
{
	...
	if (!priv->in_progress_assoc_req) {
		printf("ASSOC_RESP: no in-progress assoc request\n");
		ret = -1;
		goto done;
	}
	...
	if ( 1) {
		...
		case 0x00:
			break;
		case 0x01:
			printf("ASSOC_RESP: invalid parameters\n");
			break;
		case 0x02:
			printf("ASSOC_RESP: internal timer "
				"expired while waiting for the AP\n");
			break;
		case 0x03:
			printf("ASSOC_RESP: association "
				"refused by AP\n");
			break;
		case 0x04:
			printf("ASSOC_RESP: authentication "
				"refused by AP\n");
			break;
		default:
			printf("ASSOC_RESP: failure reason 0x%02x "
				" unknown\n", status_code);
			break;
		}
	} else {
	...
	if (status_code) {
		...
		printf("%s:wrong status code(%d)","assoc",status_code);
		do {if(0) break;}while(1);
		ret = -1;
		goto done;
	}

	lbs_hex(0x00000100, "ASSOC_RESP",
		    (void *) (resp + sizeof (resp->hdr)),
		    (resp->hdr . size) - sizeof (resp->hdr));

	 
	priv->connect_status = LBS_CONNECTED;

	 
	memcpy(&priv->curbssparams.ssid, &bss->ssid, 32);
	priv->curbssparams.ssid_len = bss->ssid_len;
	memcpy(priv->curbssparams.bssid, bss->bssid, 6);
			 ...
			 u16 command)
{
	...
	memcpy(cmd.bssid, bss->bssid, 6);
	cmd.listeninterval = (10);

	 
	tmpcap = (bss->capability & (~(0xda00)));
	if (bss->mode == 2)
		tmpcap |= (1<<0);
	cmd.capability = (tmpcap);
	printf("ASSOC_CMD: capability 0x%04x\n", tmpcap);

	 
	ssid = (struct mrvl_ie_ssid_param_set *) pos;
	ssid->header.type = (0x0000);
	tmplen = bss->ssid_len;
	ssid->header.len = (tmplen);
	memcpy(ssid->ssid, bss->ssid, tmplen);
	pos += sizeof(ssid->header) + tmplen;

	ds = (struct mrvl_ie_ds_param_set *) pos;
	ds->header.type = (0x0003);
	ds->header.len = (1);
	ds->channel = bss->phy.ds.channel;
	pos += sizeof(ds->header) + 1;

	cf = (struct mrvl_ie_cf_param_set *) pos;
	cf->header.type = (0x0004);
	tmplen = sizeof(*cf) - sizeof (cf->header);
	cf->header.len = (tmplen);
	 
	pos += sizeof(*cf);

	rates = (struct mrvl_ie_rates_param_set *) pos;
	rates->header.type = (0x0001);
	tmplen = ((((sizeof(bss->rates) / sizeof((bss->rates)[0])))<(14))? ((sizeof(bss->rates) / sizeof((bss->rates)[0]))):(14));
	...
	rates->header.len = (tmplen);
	printf("ASSOC_CMD: num rates %u\n", tmplen);
	...
	if (assoc_req->secinfo.WPAenabled || assoc_req->secinfo.WPA2enabled) {
		...
		{
			assoc_req->wpa_ie_len = priv->cur_bss->wpa_ie_len;
			memcpy(assoc_req->wpa_ie,priv->cur_bss->wpa_ie,priv->cur_bss->wpa_ie_len);
			xdbug_buf("set wpa ie",assoc_req->wpa_ie,assoc_req->wpa_ie_len);
			
		}else
		{
			
			assoc_req->wpa_ie_len = priv->cur_bss->rsn_ie_len;
			memcpy(assoc_req->wpa_ie,priv->cur_bss->rsn_ie,priv->cur_bss->rsn_ie_len);
			xdbug_buf("set rsn ie",assoc_req->wpa_ie,assoc_req->wpa_ie_len);
		}

		priv->__wpa_ie_len = assoc_req->wpa_ie_len;
		memcpy(priv->__wpa_ie,assoc_req->wpa_ie,assoc_req->wpa_ie_len);
		printf("!!!!!!!!!!!!!!! wpa_ie_len : %d , rsn_ie_len : %d.\r\n",priv->cur_bss->wpa_ie_len,priv->cur_bss->rsn_ie_len);


		
		
		memcpy(priv->__mac_addr,priv->cur_bss->bssid,6);

		 
		Ox0000008F(priv->__mac_addr,priv->current_addr,psk_out);Ox0000008D(priv->__wpa_ie,&(priv->__wpa_ie_len));;
		assoc_req->wpa_ie_len = priv->__wpa_ie_len;
		memcpy(assoc_req->wpa_ie,priv->__wpa_ie,priv->__wpa_ie_len);


		rsn = (struct mrvl_ie_rsn_param_set *) pos;
		 
		rsn->header.type = ((u16) assoc_req->wpa_ie[0]);
		tmplen = (u16) assoc_req->wpa_ie[1];
		rsn->header.len = (tmplen);
		memcpy(rsn->rsnie, &assoc_req->wpa_ie[2], tmplen);
		
		lbs_hex(0x00000200, "ASSOC_CMD: WPA IE",(u8 *)assoc_req->wpa_ie,assoc_req->wpa_ie_len);
		lbs_hex(0x00000200, "ASSOC_CMD: RSN IE", (u8 *) rsn,
			sizeof(rsn->header) + tmplen);
		pos += sizeof(rsn->header) + tmplen;
	}

	cmd.hdr.size = ((sizeof(cmd) - sizeof(cmd . iebuf)) + (u16)(pos - (u8 *) &cmd . iebuf));
	...
	struct assoc_request *assoc_req)
{
	...
	if ((priv->capability & (1<<5)) &&
	    (assoc_req->bss.capability & (1<<5)))
		preamble = 0x02;
	...
	ret = lbs_associate(priv, assoc_req, 0x0050);
					      ...
					      uint8_t *bssid, uint8_t mode)
{
	...
	struct bss_descriptor *found_bss = 0;
	...
	for (iter_bss = ((struct bss_descriptor *)(((char *)(&priv->network_list)->next) - ((unsigned long)(&((struct bss_descriptor *)0)->list)))); &iter_bss->list != (&priv->network_list); iter_bss = ((struct bss_descriptor *)(((char *)iter_bss->list . next) - ((unsigned long)(&((struct bss_descriptor *)0)->list))))) {
                              ...
                              struct assoc_request * assoc_req)
{
	...
	bss = lbs_find_bssid_in_list(priv, assoc_req->bssid,
			    assoc_req->mode);
	if (bss == 0) {
		printf("ASSOC: WAP: BSSID %pM not found, "
			"cannot associate.\n", assoc_req->bssid);
		goto out;
	}
	priv->cur_bss=bss;
	...
	if (assoc_req->mode == 2) {
		...
		printf("ASSOC: lbs_try_associate(bssid) returned %d\n",
			      ret);
	} else if (assoc_req->mode == 1) {
...
{
	u32 bestrssi = 0;
	struct bss_descriptor *iter_bss = 0;
	struct bss_descriptor *found_bss = 0;
	struct bss_descriptor *tmp_oldest = 0;
	...
	for (iter_bss = ((struct bss_descriptor *)(((char *)(&priv->network_list)->next) - ((unsigned long)(&((struct bss_descriptor *)0)->list)))); &iter_bss->list != (&priv->network_list); iter_bss = ((struct bss_descriptor *)(((char *)iter_bss->list . next) - ((unsigned long)(&((struct bss_descriptor *)0)->list))))) {
		...
		case 1:
			...
			if ((0x100 - ((u8)(iter_bss->rssi))) > bestrssi) {
				bestrssi = (0x100 - ((u8)(iter_bss->rssi)));
				found_bss = iter_bss;
			}
			break;
		case 0:
                              ...
                              struct assoc_request * assoc_req)
{
	...
	if (test_bit(2, &assoc_req->flags))
		channel = assoc_req->channel;

	printf("SSID '%s' requested\n",
	              assoc_req->ssid);
	if (assoc_req->mode == 2) {
		...
		bss = lbs_find_ssid_in_list(priv, assoc_req->ssid,
				assoc_req->ssid_len, 0, 2, channel);
		if (bss != 0) {			
			priv->cur_bss=bss;
			memcpy(&assoc_req->bss, bss, sizeof(struct bss_descriptor));
			ret = lbs_try_associate(priv, assoc_req);
		} else {
			printf("SSID not found; cannot associate\n");
		}
	} else if (assoc_req->mode == 1) {
		...
		bss = lbs_find_ssid_in_list(priv, assoc_req->ssid,
				assoc_req->ssid_len, 0, 1, channel);
		if (bss != 0) {
			priv->cur_bss=bss;
			printf("SSID found, will join\n");
			memcpy(&assoc_req->bss, bss, sizeof(struct bss_descriptor));
			lbs_adhoc_join(priv, assoc_req);
		} else {
			 
			printf("SSID not found, creating adhoc network\n");
                                  ...
                                  struct assoc_request * assoc_req)
{
	...
	if (!done && test_bit(1, &assoc_req->flags)) {
                             ...
                             struct assoc_request * assoc_req)
{
	...
	ret = lbs_set_snmp_mib(priv, 0x0000, assoc_req->mode);
                                ...
                                struct assoc_request * assoc_req)
{
	...
	if (ret) {
		printf("ASSOC: channel: error getting channel.\n");
		goto done;
	}
...
#line 795 "..\\marvel\\driver\\src\\assoc.c"
	printf("ASSOC: channel: %d -> %d\n",
		      priv->curbssparams.channel, assoc_req->channel);

	ret = lbs_set_channel(priv, assoc_req->channel);
	if (ret < 0)
		printf("ASSOC: channel: error setting channel.\n");
	...
	if (assoc_req->channel != priv->curbssparams.channel) {
		printf("ASSOC: channel: failed to update channel to %d\n",
		              assoc_req->channel);
		goto restore_mesh;
	}
	...
	if (   assoc_req->secinfo.wep_enabled
		...
		set_bit(6, &assoc_req->flags);
	}

	 
 	set_bit(1, &assoc_req->flags);
				 ...
				 struct assoc_request *assoc_req)
{
	...
	if (assoc_req->wep_keys[0].len || assoc_req->wep_keys[1].len ||
		...
		ret = lbs_cmd_802_11_set_wep(priv, 0x0002, assoc_req);
	else
		ret = lbs_cmd_802_11_set_wep(priv, 0x0004, assoc_req);
                                ...
                                struct assoc_request * assoc_req)
{
	...
	ret = lbs_cmd_802_11_enable_rsn(priv, 0x0000, &rsn);
	if (ret) {
		printf("Failed to get RSN status: %d\n", ret);
		goto out;
	}
	...
	printf("mrl set rsn action: %d\r\n",do_wpa);
	ret = lbs_cmd_802_11_enable_rsn(priv, 0x0001, &do_wpa);
	printf("mrl set rsn result: %d\r\n",do_wpa);
                                 ...
                                 struct assoc_request * assoc_req)
{
	...
	if (test_bit(9, &assoc_req->flags)) {
		clear_bit(8, &assoc_req->flags);
		ret = lbs_cmd_802_11_key_material(priv, 0x0001, assoc_req);
	...
	if (test_bit(8, &assoc_req->flags)) {
		clear_bit(9, &assoc_req->flags);
	...
	struct assoc_request *assoc_req)
{
	...
	printf("current SSID '%s', ssid length %u\n",
		priv->curbssparams.ssid,
		priv->curbssparams.ssid_len);
	printf("requested ssid '%s', ssid length %u\n",
		bss->ssid, 
		bss->ssid_len);
	...
	if (priv->curbssparams.ssid_len &&
		...
		printf("ADHOC_J_CMD: New ad-hoc SSID is the same as "
	...
	if ((priv->capability & (1<<5)) &&
	    (bss->capability & (1<<5))) {
		printf("AdhocJoin: Short preamble\n");
		preamble = 0x02;
	}
	...
	printf("AdhocJoin: channel = %d\n", assoc_req->channel);
	printf("AdhocJoin: band = %c\n", assoc_req->band);
	...
	cmd.bss.type = 0x0002;
	cmd.bss.beaconperiod = (bss->beaconperiod);

	memcpy(&cmd.bss.bssid, &bss->bssid, 6);
	...
	cmd.bss.capability = (bss->capability & (~(0xda00)));
	printf("ADHOC_J_CMD: tmpcap=%4X CAPINFO_MASK=%4X\n",
	       bss->capability, (~(0xda00)));

	 
	printf("ADHOC_J_CMD: BSSID = %pM, SSID = '%s'\n",
	...
	ratesize = ((((sizeof(cmd . bss . rates) / sizeof((cmd . bss . rates)[0])))<((sizeof(bss->rates) / sizeof((bss->rates)[0]))))? ((sizeof(cmd . bss . rates) / sizeof((cmd . bss . rates)[0]))):((sizeof(bss->rates) / sizeof((bss->rates)[0]))));
	memcpy(cmd.bss.rates, bss->rates, ratesize);
	if (get_common_rates(priv, cmd.bss.rates, &ratesize)) {
		printf("ADHOC_JOIN: get_common_rates returned error.\n");
		ret = -1;
		goto out;
	}
	...
	if (assoc_req->secinfo.wep_enabled) {
		u16 tmp = (cmd . bss . capability);
		tmp |= (1<<4);
		cmd.bss.capability = (tmp);
	}

	if (priv->psmode == LBS802_11POWERMODEMAX_PSP) {
		__le32 local_ps_mode = (LBS802_11POWERMODECAM);

		 
		ret = lbs_prepare_and_send_command(priv, 0x0021,
						   0x0001, 0, 0,
						   &local_ps_mode);
		if (ret) {
			ret = -1;
	...
	ret = lbs_cmd_with_response(priv, 0x002c, &cmd);
	if (ret == 0) {
	...
	struct assoc_request *assoc_req)
{
	...
	if (priv->capability & (1<<5)) {
		printf("ADHOC_START: Will use short preamble\n");
		preamble = 0x02;
	}
	...
	printf("ADHOC_START: SSID '%s', ssid length %u\n",
		assoc_req->ssid, 
		assoc_req->ssid_len);
	...
	cmd.beaconperiod = (priv->beacon_period);
	...
	if (assoc_req->secinfo.wep_enabled ||
		...
		printf("ADHOC_START: WEP/WPA enabled, privacy on\n");
		tmpcap |= (1<<4);
	} else
		printf("ADHOC_START: WEP disabled, privacy off\n");
	...
	ratesize = (((sizeof(cmd . rates))<(sizeof(lbs_bg_rates)))? (sizeof(cmd . rates)):(sizeof(lbs_bg_rates)));
	...
	printf("ADHOC_START: rates=%02x %02x %02x %02x\n",
	...
	printf("ADHOC_START: Starting Ad-Hoc BSS on channel %d, band %d\n",
		     assoc_req->channel, assoc_req->band);

	priv->adhoccreate = 1;
	priv->mode = 1;

	ret = lbs_cmd_with_response(priv, 0x002b, &cmd);
	if (ret == 0)
		ret = lbs_adhoc_post(priv,
				     (struct cmd_ds_802_11_ad_hoc_result *)&cmd);
...
int lbs_adhoc_stop(struct lbs_private *priv)
{
	...
	ret = lbs_cmd_with_response(priv, 0x0040, &cmd);
...
{
	int ret = 0;
	u16 command = (resp->hdr . command);
	u16 result = (resp->hdr . result);
	union iwreq_data wrqu;
	struct bss_descriptor *bss;
	...
	if (!priv->in_progress_assoc_req) {
		printf("ADHOC_RESP: no in-progress association "
			"request\n");
		ret = -1;
		goto done;
	}
	...
	if (result) {
		printf("ADHOC_RESP: failed (result 0x%X)\n", result);
	...
	if (command == (0x8000 | 0x002b)) {
		 
		memcpy(bss->bssid, resp->bssid, 6);
	}
	...
	printf("ADHOC_RESP: Joined/started '%s', BSSID %pM, channel %d\n",
		     bss->ssid,
...
static int set_bssinfo_to_priv(struct lbs_private *priv,
				 struct bss_descriptor *bss)
{
				 ...
				 struct bss_descriptor *bss, uint8_t mode)
{
	...
	if (matched) {
		printf("is_network_compatible() WPA: wpa_ie 0x%x "
			     ...
			     (bss->capability & (1<<4)));
		set_bssinfo_to_priv(priv,bss);
		goto done;
	}
	matched = match_bss_wpa2(&priv->secinfo, bss);
	if (matched) {
		printf("is_network_compatible() WPA2: wpa_ie 0x%x "
			     "wpa2_ie 0x%x WEP %s WPA %s WPA2 %s "
			     "privacy 0x%x\n", bss->wpa_ie[0], bss->rsn_ie[0],
			     priv->secinfo.wep_enabled ? "e" : "d",
			     priv->secinfo.WPAenabled ? "e" : "d",
	...
	if (matched) {
		printf("is_network_compatible() dynamic WEP: "
	...
	printf("is_network_compatible() FAILED: wpa_ie 0x%x "
	...
	struct lbs_private *priv, uint8_t mode)
{
	uint8_t bestrssi = 0;
	struct bss_descriptor *iter_bss;
	struct bss_descriptor *best_bss = 0;
                                        ...
                                        struct assoc_request * assoc_req)
{
	...
	;
	if (test_bit(1, &assoc_req->flags)) {
		printf("Deauthenticating due to new SSID\n");
		ret = 1;
		goto out;
	}

	if (test_bit(10, &assoc_req->flags)) {
		if (priv->secinfo.auth_mode != assoc_req->secinfo.auth_mode) {
			printf("Deauthenticating due to new security\n");
			ret = 1;
			goto out;
		}
	}
	...
	if (test_bit(5, &assoc_req->flags)) {
		printf("Deauthenticating due to new BSSID\n");
		ret = 1;
		goto out;
	}

	if (test_bit(2, &assoc_req->flags)) {
		printf("Deauthenticating due to channel switch\n");
		ret = 1;
		goto out;
	}

	 
	if (test_bit(4, &assoc_req->flags)) {
		if (assoc_req->mode != 2) {
			printf("Deauthenticating due to leaving "
				"infra mode\n");
			ret = 1;
			goto out;
		}
...
#line 1714 "..\\marvel\\driver\\src\\assoc.c"
char * psk_out;
static int xxx_find_ssid(struct lbs_private *priv , struct assoc_request * assoc , char *ssid , char *key)
{
	struct bss_descriptor *iter_bss;
	struct bss_descriptor *found_bss = 0;
	...
	for (iter_bss = ((struct bss_descriptor *)(((char *)(&priv->network_list)->next) - ((unsigned long)(&((struct bss_descriptor *)0)->list)))); &iter_bss->list != (&priv->network_list); iter_bss = ((struct bss_descriptor *)(((char *)iter_bss->list . next) - ((unsigned long)(&((struct bss_descriptor *)0)->list))))) {
		...
		if (strcmp(iter_bss->ssid,ssid) == 0)
		{
			...
			if (iter_bss->wpa_ie[0] == WLAN_EID_GENERIC) 
			{
				assoc->secinfo.wep_enabled=0;
				assoc->secinfo.WPAenabled=1;
			...
			if (iter_bss->rsn_ie[0] == WLAN_EID_RSN) 
			{
				assoc->secinfo.wep_enabled=0;
				assoc->secinfo.WPA2enabled=1;
			...
			if (assoc->secinfo.WPAenabled || assoc->secinfo.WPA2enabled)
			{
		
				
				pbkdf2_sha1(marvel_key, marvel_ssid, strlen(marvel_ssid), 4096, psk_out, 32);
				assoc->psk = psk_out;
				printf("WPA password %s , %s \n",marvel_key,psk_out);
												
			}else
			{
				extern int newpower_time_out_current_state;
				newpower_time_out_current_state = 8; 
			}
...
{	
	...
	printf("please input essid:%s\n",ssid);
	printf("please input wep/wpa key:%s\n",key);
    memcpy(assoc->ssid,ssid,strlen(ssid));
    assoc->ssid_len=strlen((const char *)assoc->ssid);


	if(strlen(key) && assoc->secinfo.WPAenabled!=1 && assoc->secinfo.WPA2enabled!=1){
	...
	char *ssid,char *key,char mode)
{
	...
	memset(&gmarvel_adhoc_assoc,0,sizeof(struct assoc_request));
	xxx_find_ssid(priv,&gmarvel_adhoc_assoc,ssid,key);
...
void lbs_association_worker(struct lbs_private *priv)
{
	struct assoc_request * assoc_req = 0;
	char ssid[10];
	int ret = 0;
	int find_any_ssid = 0;
	...
	wpa_printf(MSG_DEBUG,
		...
		(assoc_req->bssid)[0], (assoc_req->bssid)[1], (assoc_req->bssid)[2], (assoc_req->bssid)[3], (assoc_req->bssid)[4], (assoc_req->bssid)[5],
	...
	if (find_any_ssid) {
		...
		if (ret) {
			printf("Could not find best network\n");
			ret = -51;
			goto out;
		}
	...
	if (priv->mode == 2) {
		if (should_deauth_infrastructure(priv, assoc_req)) {
			...
			if (ret) {
				printf("Deauthentication due to new "
		...
		if (should_stop_adhoc(priv, assoc_req)) {
			...
			if (ret) {
				printf("Teardown of AdHoc network due to "
	...
	if ( test_bit(6, &assoc_req->flags)
	    || test_bit(7, &assoc_req->flags)) {
	...
	if (test_bit(11, &assoc_req->flags)) {
	...
	if (test_bit(5, &assoc_req->flags)
		...
		int success = 1;
		 ...
		 if (assoc_req->mode == 0)
		 {
        	...
        	clear_wpa_wpa2_info(priv);
		 }

		ret = assoc_helper_associate(priv, assoc_req);
		if (ret) {
			printf("ASSOC: association unsuccessful: %d\n",
				ret);
			success = 0;
		}

		if (priv->connect_status != LBS_CONNECTED) {
			printf("ASSOC: association unsuccessful, "
				"not connected\n");
			success = 0;
		}
...
out:
	if (ret) {
		printf("ASSOC: reconfiguration attempt unsuccessful: %d\n",
			ret);
	}
	priv->in_progress_assoc_req = 0;
done:
	;
}
				 ...
				 u16 reason)
{
	...
	memset(&cmd, 0, sizeof(cmd));
	cmd.hdr.size = (sizeof(cmd));
	memcpy(cmd.macaddr, &bssid[0], 6);
	cmd.reasoncode = (reason);

	ret = lbs_cmd_with_response(priv, 0x0024, &cmd);
