#line 1 "..\\marvel\\driver\\src\\scan.c"
...
#line 2 "..\\marvel\\driver\\src\\scan.c"
...
#line 3 "..\\marvel\\driver\\src\\scan.c"
...
#line 4 "..\\marvel\\driver\\src\\scan.c"
...
#line 5 "..\\marvel\\driver\\src\\scan.c"
...
#line 6 "..\\marvel\\driver\\src\\scan.c"
#line 7 "..\\marvel\\driver\\src\\scan.c"
...
#line 8 "..\\marvel\\driver\\src\\scan.c"
...
#line 27 "..\\marvel\\driver\\src\\scan.c"
					...
					struct chanscanparamset *scanchanlist)
{
	...
	scantype = 0x0000;
	 ...
	 for (rgnidx = 0; rgnidx < (sizeof(priv->region_channel) / sizeof((priv->region_channel)[0])); rgnidx++) {
	

#line 124 "..\\marvel\\driver\\src\\scan.c"
		...
		for (nextchan = 0; nextchan < scanregion->nrcfp; nextchan++, chanidx++) {
			...
			if (scanregion->band == (0x01) || scanregion->band == (0x02))
				chan->radiotype = 0;

			if (scantype == 0x0001) {
				chan->maxscantime = (100);
...
static int lbs_scan_add_ssid_tlv(struct lbs_private *priv, u8 *tlv)
{
	...
	ssid_tlv->header.type = (0x0000);
	ssid_tlv->header.len = (priv->scan_ssid_len);
				     ...
				     int chan_count)
{
	...
	chan_tlv->header.type = ((0x0100 + 1));
	memcpy(chan_tlv->chanscanparam, chan_list, size);
	chan_tlv->header.len = (size);
...
static __inline void clear_bss_descriptor(struct bss_descriptor *bss)
{
	...
	memset(bss, 0, ((size_t) &((struct bss_descriptor *)0)->list));
}
			   ...
			   uint8_t **pbeaconinfo, int *bytesleft)
{
	...
	*bytesleft -= beaconsize;
	...
	memcpy(bss->bssid, pos, 6);
	printf("process_bss: BSSID %2x:%2x:%2x:%2x:%2x:%2x\n", 
	...
	if ((end - pos) < 12) {
		printf("process_bss: Not enough bytes left\n");
		ret = -1;
		goto done;
	}
	...
	bss->beaconperiod = ((__le16)(*pos));
	pos += 2;

	 
	bss->capability = ((__le16)(*pos));
	printf("process_bss: capabilities 0x%04x\n", bss->capability);
	pos += 2;

	if (bss->capability & (1<<4))
		printf("process_bss: WEP enabled\n");
	else
		printf("process_bss: WEP off\n");
	...
	printf("process_bss: IE len %zd\n", end - pos);
	

	 
	while (pos <= end - 2) {
		if (pos + pos[1] > end) {
			printf("process_bss: error in processing IE, "
		...
		switch (pos[0]) {
		case WLAN_EID_SSID:
			bss->ssid_len = (((32)<(pos[1]))? (32):(pos[1]));
			memcpy(bss->ssid, pos + 2, bss->ssid_len);
			printf("got SSID IE: '%s', len %u\n",
		...
		case WLAN_EID_SUPP_RATES:
			n_basic_rates = (((14)<(pos[1]))? (14):(pos[1]));
		...
		case WLAN_EID_EXT_SUPP_RATES:
			...
			printf("got RATESEX IE\n");
			if (!got_basic_rates) {
				printf("... but ignoring it\n");
				break;
			}

			n_ex_rates = pos[1];
			if (n_basic_rates + n_ex_rates > 14)
				n_ex_rates = 14 - n_basic_rates;
		...
		case WLAN_EID_GENERIC:
			if (pos[1] >= 4 &&
				...
				bss->wpa_ie_len = (((pos[1] + 2)<(64))? (pos[1] + 2):(64));
				...
				printf("got generic IE: %02x:%02x:%02x:%02x, len %d\n",
					pos[2], pos[3],
					pos[4], pos[5],
					pos[1]);
			}
		...
		case WLAN_EID_RSN:
			printf("got RSN IE\n");
			bss->rsn_ie_len = (((pos[1] + 2)<(64))? (pos[1] + 2):(64));
			      ...
			      struct cmd_header *resp)
{
	...
	if (scanresp->nr_sets > 12) {
		printf("SCAN_RESP: too many scan results (%d, max %d)\n",
			     scanresp->nr_sets, 12);
		ret = -1;
		goto done;
	}

	bytesleft = ((__le16)(* &scanresp->bssdescriptsize));
	printf("SCAN_RESP: bssdescriptsize %d\n", bytesleft);

	scanrespsize = (resp->size);
	printf("SCAN_RESP: scan results %d\n", scanresp->nr_sets);
	...
	for (idx = 0; idx < scanresp->nr_sets && bytesleft; idx++) {
		...
		struct bss_descriptor *found = 0;
		struct bss_descriptor *oldest = 0;
		 
		memset(&new, 0, sizeof (struct bss_descriptor));
		if (lbs_process_bss(&new, &bssinfo, &bytesleft) != 0) {
			 
			printf("SCAN_RESP: process_bss returned ERROR(%d)\n",tlvbufsize);
			continue;
		}
		...
		if (found) {
			...
			found = ((struct bss_descriptor *)(((char *)priv->network_free_list . next) - ((unsigned long)(&((struct bss_descriptor *)0)->list))));
		...
		printf("SCAN_RESP: BSSID %2x:%2x:%2x:%2x:%2x:%2x\n",
				new.bssid[0],new.bssid[1],new.bssid[2],new.bssid[3],
				new.bssid[4],new.bssid[5]);

		 
		memcpy(found, &new, ((size_t) &((struct bss_descriptor *)0)->list));
	}
...
{
	int ret = -12;
	u8 gscan_cmdbuf[(sizeof(struct cmd_ds_802_11_scan) + (sizeof(struct mrvl_ie_header) + (14 * sizeof(struct chanscanparamset))) + (1 * sizeof(struct mrvl_ie_ssid_param_set)))];
	...
	printf( "bsstype %d, chanlist[].chan %d, chan_count %d\n",
	...
	memset(scan_cmd,0,(sizeof(struct cmd_ds_802_11_scan) + (sizeof(struct mrvl_ie_header) + (14 * sizeof(struct chanscanparamset))) + (1 * sizeof(struct mrvl_ie_ssid_param_set))));
	...
	scan_cmd->hdr.size = (tlv - (uint8_t *)scan_cmd);
	...
	xdbug_buf("dbug tlv buf",scan_cmd->tlvbuffer,tlv - scan_cmd->tlvbuffer);
	...
	ret = __lbs_cmd(priv, 0x0006, &scan_cmd->hdr,
			(scan_cmd->hdr . size),
...
{
	int ret = -12;
	struct chanscanparamset gmarvel_scan_param[50];
	struct chanscanparamset *chan_list;
	struct chanscanparamset *curr_chans;
	int chan_count;
	uint8_t bsstype = 0x0003;
	int numchannels = 4;
	...
	printf("numchannels %d, bsstype %d\n", numchannels, bsstype);
	...
	memset(chan_list,0,sizeof(struct chanscanparamset) *			50);
	...
	printf("chan_count %d, scan_channel %d\n",
	...
	while (chan_count) {
		int to_scan = (((numchannels)<(chan_count))? (numchannels):(chan_count));
		printf("scanning %d of %d channels\n",
			     to_scan, chan_count);
		ret = lbs_do_scan(priv, bsstype, curr_chans,
				  to_scan);
		if (ret) {
			printf("SCAN_CMD failed\n");
			goto out2;
		}
		curr_chans += to_scan;
		chan_count -= to_scan;
...
#line 918 "..\\marvel\\driver\\src\\scan.c"
	...
	printf("scan table:\n");
	for (iter = ((struct bss_descriptor *)(((char *)(&priv->network_list)->next) - ((unsigned long)(&((struct bss_descriptor *)0)->list)))); &iter->list != (&priv->network_list); iter = ((struct bss_descriptor *)(((char *)iter->list . next) - ((unsigned long)(&((struct bss_descriptor *)0)->list)))))
		printf("%02d: BSSID %pM, RSSI %d, SSID '%s'\n",
