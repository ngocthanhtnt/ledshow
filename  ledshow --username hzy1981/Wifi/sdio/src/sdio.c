...
int mmc_attach_sdio(struct mmc_host *host, u32 ocr)
{
	 ...
	 */
	 printk("device function count=%d", funcs);
	for (i = 0;i < funcs;i++) {
		err = sdio_init_func(host->card, i + 1);
		if (err)
			goto remove;
	}
	printk("device function inital is ok!\n");
...
static int process_sdio_pending_irqs(struct mmc_card *card)
{
	...
	if (ret) {
		printk(KERN_DEBUG "error %d reading SDIO_CCCR_INTx\n",ret);
		return ret;
	}
	...
	for (i = 1; i <= 7; i++) {
		if (pending & (1 << i)) {
			...
			if (!func) {
				printk(KERN_WARNING "pending IRQ for "
				...
				printk(KERN_WARNING "pending IRQ with no handler\n");
				ret = -EINVAL;
			}
		}
	}
