...
typedef struct
{
	...
	INT16U Connect_Period; //重连周?

  INT8U Mode1; //GPRS工作模式,唤醒模式，或者GPRS模式
	INT8U APN[30]; //APN
  
	INT8U Temp[11]; //保留11字节备用--升级程序时可以和老的兼容
}S_Net_Para;
...
typedef struct
{
    ...
    INT16U Heart_Period; //心跳周期
    INT16U Connect_Period; //重连周期
