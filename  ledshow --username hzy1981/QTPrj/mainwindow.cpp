...
QIcon getTypeIcon(INT8U type)
{
  ...
  if(type EQ SCREEN_PROPERTY)
     ...
     icon = QIcon::fromTheme(QObject::tr("内码"), QIcon(rsrcPath1 + QObject::tr("/字幕.png")));
  return icon;
}
 ...
 */
    ...
    QIcon txtIcon = getTypeIcon(TXT_PROPERTY);//QIcon::fromTheme(tr("字幕"), QIcon(rsrcPath1 + tr("/字幕.ico")));
    actionTxt = a = new QAction(txtIcon, tr("内码"), this);
    //a->setShortcut(QKeySequence::Save);
    actionTxt->setEnabled((bool)Get_Txt_Show_En());//(bool)Get_Pic_Show_En());
    connect(a, SIGNAL(triggered()), progManage, SLOT(newTxt())); //新建节目
    tb->addAction(a);
    menu->addAction(a);
