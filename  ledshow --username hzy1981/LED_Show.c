...
//获取缓冲区中第Index位的值
INT8U Get_Buf_Bit(INT8U Buf[], INT16U Buf_Size, INT16U Index)
{
  if(Index >= Buf_Size * 8)
  {
    ASSERT_FAILED();
                      ...
                      INT16S Angle, INT16U Len, INT16U Radius, INT8U Value)
{
   ...
   Fill_Round(pDst_Buf, Area_No, pCenter, Radius, Value); //绘制一个圆
}
/*
//Buf到分区的映射
                     ...
                     INT16S Angle, INT16U Len, INT16U Width, INT8U Value)
{
  ...
  Get_Angle_Point(pCenter, Angle, Len * 8 / 10, &Point0); //前端的顶点
  ...
  Get_Angle_Point(pCenter, Angle + 180, Len * 2 / 10, &Point0); //后端的顶点
...
void Show_Clock(S_Show_Data *pDst_Buf, INT8U Area_No, S_Time *pTime, S_Clock_Para *pClock_Para)
{
   ...
   //----------至此0-11点所有的点都已经绘制完成------------
   //时钟
   Fill_Clock_Line(pDst_Buf, Area_No, &Point, 360 * pTime->Time[T_HOUR] % 12 / 12, Radius * 6 / 10,\
                   pClock_Para->Hour_Line_Width, pClock_Para->Hour_Line_Color);  
   //分钟
   Fill_Clock_Line(pDst_Buf, Area_No, &Point, 360 * pTime->Time[T_MIN] / 60, Radius * 8 / 10,\
                   pClock_Para->Min_Line_Width, pClock_Para->Min_Line_Color);
   //秒钟
   Fill_Clock_Line(pDst_Buf, Area_No, &Point, 360 * pTime->Time[T_SEC] / 60, Radius * 9 / 10,\
...
void Update_Clock_Data(INT8U Area_No)
{
  ...
  Copy_Rect(&Show_Data_Bak, Area_No, &Point, X_Len, Y_Len, &Show_Data, &Point);  
}
