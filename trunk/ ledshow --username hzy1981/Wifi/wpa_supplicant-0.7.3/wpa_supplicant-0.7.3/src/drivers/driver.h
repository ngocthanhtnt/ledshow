...
struct wpa_driver_ops {
	 ...
	 * string.
	 */
	int (*get_ssid)(void *priv, u8 *ssid);									//4
	 ...
	 */
	int (*set_key)(const char *ifname, void *priv, enum wpa_alg alg,		 //5
	 ...
	 */
	void * (*init)(void *ctx, const char *ifname);							 //6
	 ...
	 */
	int (*set_param)(void *priv, const char *param);						//8
	 ...
	 */
	int (*set_countermeasures)(void *priv, int enabled);				  //9
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*deauthenticate)(void *priv, const u8 *addr, int reason_code);	  //10
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*disassociate)(void *priv, const u8 *addr, int reason_code);	   //11
	...
	int (*associate)(void *priv,
			 struct wpa_driver_associate_params *params);				   //12
	 ...
	 */
	int (*add_pmkid)(void *priv, const u8 *bssid, const u8 *pmkid);		  //13
	 ...
	 */
	int (*remove_pmkid)(void *priv, const u8 *bssid, const u8 *pmkid);	   //14
	 ...
	 */
	int (*get_capa)(void *priv, struct wpa_driver_capa *capa);	  //16
	 ...
	 */
	const char * (*get_ifname)(void *priv);					  //18
	 ...
	 * a MAC address. */
	const u8 * (*get_mac_addr)(void *priv);					  //19
	...
	int (*send_eapol)(void *priv, const u8 *dest, u16 proto,
			  const u8 *data, size_t data_len);						   //20
	 ...
	 */
	int (*set_operstate)(void *priv, int state);					  //21
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*set_ssid)(void *priv, const u8 *ssid, size_t ssid_len);	   //25
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*set_bssid)(void *priv, const u8 *bssid);				 //26
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*send_mlme)(void *priv, const u8 *data, size_t data_len);		//27
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*mlme_remove_sta)(void *priv, const u8 *addr);				  //29
	...
	int (*send_ft_action)(void *priv, u8 action, const u8 *target_ap,
			      const u8 *ies, size_t ies_len);					   //31
	 ...
	 */
	 struct wpa_scan_results * (*get_scan_results2)(void *priv);	   //32
	 ...
	 */
	int (*set_country)(void *priv, const char *alpha2);				  //33
	 ...
	 */
	void (*global_deinit)(void *priv);									   //35
	 ...
	 * uses global data.
	 */
	void * (*init2)(void *ctx, const char *ifname, void *global_priv);		 //36
	 ...
	 * failure
	 */
	struct wpa_interface_info * (*get_interfaces)(void *global_priv);		 //37
	 ...
	 */
	int (*scan2)(void *priv, struct wpa_driver_scan_params *params);		//38
	...
	int (*authenticate)(void *priv,
			    struct wpa_driver_auth_params *params);					 //39
	 ...
	 * for Beacon frames.
	 */
	int (*set_ieee8021x)(void *priv, struct wpa_bss_params *params);	   //43
	 ...
	 */
	int (*set_privacy)(void *priv, int enabled);						   //44
	 ...
	 * set_beacon().
	 */
	int (*set_generic_elem)(void *priv, const u8 *elem, size_t elem_len);	//47
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*sta_remove)(void *priv, const u8 *addr);					//52
	 ...
	 * frames.
	 */
	int (*hapd_get_ssid)(void *priv, u8 *buf, int len);				//53
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*hapd_set_ssid)(void *priv, const u8 *buf, int len);		//54
	 ...
	 */
	int (*hapd_set_countermeasures)(void *priv, int enabled);	   //55
	 ...
	 */
	int (*sta_add)(void *priv, struct hostapd_sta_add_params *params);	  //56
	 ...
	 */
	int (*get_inact_sec)(void *priv, const u8 *addr);				 //57
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*sta_clear_stats)(void *priv, const u8 *addr);			   //58
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_freq)(void *priv, struct hostapd_freq_params *freq);	//59
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_rts)(void *priv, int rts);							//60
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_frag)(void *priv, int frag);						   //61
	...
	int (*sta_set_flags)(void *priv, const u8 *addr,
			     int total_flags, int flags_or, int flags_and);		 //62
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_cts_protect)(void *priv, int value);						   //64
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_preamble)(void *priv, int value);							//65
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_short_slot_time)(void *priv, int value);					//66
	 ...
	 */
	int (*valid_bss_mask)(void *priv, const u8 *addr, const u8 *mask);			 //68
	...
	int (*if_add)(void *priv, enum wpa_driver_if_type type,
		      ...
		      void **drv_priv, char *force_ifname, u8 *if_addr);			  //69
	...
	int (*send_ether)(void *priv, const u8 *dst, const u8 *src, u16 proto,
			  const u8 *data, size_t data_len);									  //73
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_radius_acl_expire)(void *priv, const u8 *mac);						 //75
	...
	int (*set_ht_params)(void *priv,
			     ...
			     const u8 *ht_oper, size_t ht_oper_len);							 //76
	...
	int (*set_ap_wps_ie)(void *priv, const struct wpabuf *beacon,
			     const struct wpabuf *proberesp);								   //77
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_supp_port)(void *priv, int authorized);							  //78
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_wds_sta)(void *priv, const u8 *addr, int aid, int val);			 //79
	...
	int (*send_action)(void *priv, unsigned int freq,
			   ...
			   const u8 *data, size_t data_len);									//80
	 ...
	 */
	int (*cancel_remain_on_channel)(void *priv);								   //82
	 ...
	 */
	int (*probe_req_report)(void *priv, int report);							   //83
	 ...
	 */
	int (*disable_11b_rates)(void *priv, int disabled);							  //84
	 ...
	 */
	int (*deinit_ap)(void *priv);												   //85

	/**
	 * suspend - Notification on system suspend/hibernate event
	 * @priv: Private driver interface data
	 */
	void (*suspend)(void *priv);													//86

	/**
	 * resume - Notification on system resume/thaw event
	 * @priv: Private driver interface data
	 */
	void (*resume)(void *priv);														//87
	 ...
	 */
	int (*signal_monitor)(void *priv, int threshold, int hysteresis);				//88
