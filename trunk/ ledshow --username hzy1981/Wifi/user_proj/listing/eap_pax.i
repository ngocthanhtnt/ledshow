#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_pax.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_pax.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_pax.c"
#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common/eap_pax_common.h"
...
struct eap_pax_hdr {
	...
	unsigned char dh_group_id;
	unsigned char public_key_id;
...
#line 83 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common/eap_pax_common.h"


int eap_pax_kdf(unsigned char mac_id, const unsigned char *key, size_t key_len,
		const char *identifier,
		const unsigned char *entropy, size_t entropy_len,
		size_t output_len, unsigned char *output);
int eap_pax_mac(unsigned char mac_id, const unsigned char *key, size_t key_len,
		...
		unsigned char *mac);
int eap_pax_initial_key_derivation(unsigned char mac_id, const unsigned char *ak, const unsigned char *e,
				   unsigned char *mk, unsigned char *ck, unsigned char *ick);

#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_pax.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_pax.c"
...
struct eap_pax_data {
	...
	unsigned char mac_id, dh_group_id, public_key_id;
	union {
		unsigned char e[2 * 32];
		struct {
			unsigned char x[32];  
			unsigned char y[32];  
		} r;
...
static void * eap_pax_init(struct eap_sm *sm)
{
	...
	if (!identity || !password) {
		wpa_printf(0, "EAP-PAX: CID (nai) or key (password) "
			   "not configured");
		return 0;
	}

	if (password_len != 16) {
		wpa_printf(0, "EAP-PAX: Invalid PSK length");
		return 0;
	}
...
static struct wpabuf * eap_pax_alloc_resp(const struct eap_pax_hdr *req,
					  unsigned char id, unsigned char op_code, size_t plen)
{
	struct wpabuf *resp;
					     ...
					     size_t req_plen)
{
	...
	wpa_printf(0, "EAP-PAX: PAX_STD-1 (received)");

	if (data->state != PAX_INIT) {
		wpa_printf(0, "EAP-PAX: PAX_STD-1 received in "
	...
	if (req->flags & 0x02) {
		wpa_printf(0, "EAP-PAX: PAX_STD-1 with CE flag set - "
			   "ignored");
		ret->ignore = 1;
		return 0;
	}
	...
	if (left < 2 + 32) {
		wpa_printf(0, "EAP-PAX: PAX_STD-1 with too short "
			   "payload");
		ret->ignore = 1;
		return 0;
	}
	...
	if (((unsigned short) (((pos)[0] << 8) | (pos)[1])) != 32) {
		wpa_printf(0, "EAP-PAX: PAX_STD-1 with incorrect A "
			   "length %d (expected %d)",
			   ((unsigned short) (((pos)[0] << 8) | (pos)[1])), 32);
		ret->ignore = 1;
		return 0;
	}
	...
	pos += 2;
	left -= 2;
	os_memcpy(data->rand.r.x, pos, 32);
	wpa_hexdump(0, "EAP-PAX: X (server rand)",
		    data->rand.r.x, 32);
	pos += 32;
	left -= 32;

	if (left > 0) {
		wpa_hexdump(0, "EAP-PAX: ignored extra payload",
			    pos, left);
	}

	if (os_get_random(data->rand.r.y, 32)) {
		wpa_printf(0, "EAP-PAX: Failed to get random data");
		ret->ignore = 1;
		return 0;
	}
	wpa_hexdump(0, "EAP-PAX: Y (client rand)",
		    data->rand.r.y, 32);
	...
	wpa_printf(0, "EAP-PAX: PAX_STD-2 (sending)");

	plen = 2 + 32 + 2 + data->cid_len + 2 + 16 +
		16;
	resp = eap_pax_alloc_resp(req, id, EAP_PAX_OP_STD_2, plen);
	if (resp == 0)
		return 0;

	wpabuf_put_be16(resp, 32);
	wpabuf_put_data(resp, data->rand.r.y, 32);
	wpa_hexdump(0, "EAP-PAX: B = Y (client rand)",
		    data->rand.r.y, 32);

	wpabuf_put_be16(resp, data->cid_len);
	wpabuf_put_data(resp, data->cid, data->cid_len);
	wpa_hexdump_ascii(0, "EAP-PAX: CID",
			  (unsigned char *) data->cid, data->cid_len);

	wpabuf_put_be16(resp, 16);
	rpos = wpabuf_put(resp, 16);
	eap_pax_mac(req->mac_id, data->ck, 16,
		    data->rand.r.x, 32,
		    data->rand.r.y, 32,
		    (unsigned char *) data->cid, data->cid_len, rpos);
	wpa_hexdump(0, "EAP-PAX: MAC_CK(A, B, CID)",
		    rpos, 16);

	 

	rpos = wpabuf_put(resp, 16);
	eap_pax_mac(req->mac_id, data->ick, 16,
		    wpabuf_head(resp), wpabuf_len(resp) - 16,
		    0, 0, 0, 0, rpos);
	wpa_hexdump(0, "EAP-PAX: ICV", rpos, 16);
...
{
	struct wpabuf *resp;
	unsigned char *rpos, mac[16];
	const unsigned char *pos;
	size_t left;

	wpa_printf(0, "EAP-PAX: PAX_STD-3 (received)");

	if (data->state != PAX_STD_2_SENT) {
		wpa_printf(0, "EAP-PAX: PAX_STD-3 received in "
			   "unexpected state (%d) - ignored", data->state);
		ret->ignore = 1;
		return 0;
	}
	...
	if (req->flags & 0x02) {
		wpa_printf(0, "EAP-PAX: PAX_STD-3 with CE flag set - "
			   "ignored");
		ret->ignore = 1;
		return 0;
	}
	...
	if (left < 2 + 16) {
		wpa_printf(0, "EAP-PAX: PAX_STD-3 with too short "
			   "payload");
		ret->ignore = 1;
		return 0;
	}
	...
	pos = (const unsigned char *) (req + 1);
	if (((unsigned short) (((pos)[0] << 8) | (pos)[1])) != 16) {
		wpa_printf(0, "EAP-PAX: PAX_STD-3 with incorrect "
			   "MAC_CK length %d (expected %d)",
			   ((unsigned short) (((pos)[0] << 8) | (pos)[1])), 16);
		ret->ignore = 1;
		return 0;
	}
	pos += 2;
	left -= 2;
	wpa_hexdump(0, "EAP-PAX: MAC_CK(B, CID)",
		    pos, 16);
	eap_pax_mac(data->mac_id, data->ck, 16,
		    data->rand.r.y, 32,
		    (unsigned char *) data->cid, data->cid_len, 0, 0, mac);
	if (os_memcmp(pos, mac, 16) != 0) {
		wpa_printf(0, "EAP-PAX: Invalid MAC_CK(B, CID) "
			   "received");
		wpa_hexdump(0, "EAP-PAX: expected MAC_CK(B, CID)",
			    mac, 16);
	...
	wpa_printf(0, "EAP-PAX: PAX-ACK (sending)");

	resp = eap_pax_alloc_resp(req, id, EAP_PAX_OP_ACK, 16);
	if (resp == 0)
		return 0;
	...
	rpos = wpabuf_put(resp, 16);
	eap_pax_mac(data->mac_id, data->ick, 16,
		    wpabuf_head(resp), wpabuf_len(resp) - 16,
		    0, 0, 0, 0, rpos);
	wpa_hexdump(0, "EAP-PAX: ICV", rpos, 16);
				       ...
				       const struct wpabuf *reqData)
{
	...
	const struct eap_pax_hdr *req;
	struct wpabuf *resp;
	unsigned char icvbuf[16], id;
	const unsigned char *icv, *pos;
	size_t len;
	unsigned short flen, mlen;
	...
	flen = len - 16;
	mlen = wpabuf_len(reqData) - 16;

	wpa_printf(0, "EAP-PAX: received frame: op_code 0x%x "
		   "flags 0x%x mac_id 0x%x dh_group_id 0x%x "
		   "public_key_id 0x%x",
		   req->op_code, req->flags, req->mac_id, req->dh_group_id,
		   req->public_key_id);
	wpa_hexdump(0, "EAP-PAX: received payload",
		    pos, len - 16);

	if (data->state != PAX_INIT && data->mac_id != req->mac_id) {
		wpa_printf(0, "EAP-PAX: MAC ID changed during "
	...
	if (data->state != PAX_INIT && data->dh_group_id != req->dh_group_id) {
		wpa_printf(0, "EAP-PAX: DH Group ID changed during "
			   "authentication (was 0x%d, is 0x%d)",
			   data->dh_group_id, req->dh_group_id);
		ret->ignore = 1;
		return 0;
	...
	if (data->state != PAX_INIT &&
	    data->public_key_id != req->public_key_id) {
		wpa_printf(0, "EAP-PAX: Public Key ID changed during "
			   "authentication (was 0x%d, is 0x%d)",
			   data->public_key_id, req->public_key_id);
		ret->ignore = 1;
		return 0;
	...
	if (req->mac_id != 0x01) {
		wpa_printf(0, "EAP-PAX: Unsupported MAC ID 0x%x",
			   req->mac_id);
		ret->ignore = 1;
		return 0;
	}

	if (req->dh_group_id != 0x00) {
		wpa_printf(0, "EAP-PAX: Unsupported DH Group ID 0x%x",
			   req->dh_group_id);
		ret->ignore = 1;
		return 0;
	}

	if (req->public_key_id != 0x00) {
		wpa_printf(0, "EAP-PAX: Unsupported Public Key ID 0x%x",
			   req->public_key_id);
		ret->ignore = 1;
		return 0;
	}
	...
	if (req->flags & 0x01) {
		...
		wpa_printf(0, "EAP-PAX: fragmentation not supported - "
			   "ignored packet");
		ret->ignore = 1;
		return 0;
	}
	...
	icv = pos + len - 16;
	wpa_hexdump(0, "EAP-PAX: ICV", icv, 16);
	if (req->op_code == EAP_PAX_OP_STD_1) {
		eap_pax_mac(req->mac_id, (unsigned char *) "", 0,
			    wpabuf_head(reqData), mlen, 0, 0, 0, 0,
			    icvbuf);
	} else {
		eap_pax_mac(req->mac_id, data->ick, 16,
			    wpabuf_head(reqData), mlen, 0, 0, 0, 0,
	...
	if (os_memcmp(icv, icvbuf, 16) != 0) {
		wpa_printf(0, "EAP-PAX: invalid ICV - ignoring the "
			   "message");
		wpa_hexdump(0, "EAP-PAX: expected ICV",
			    icvbuf, 16);
		ret->ignore = 1;
		return 0;
	}
	...
	case EAP_PAX_OP_STD_3:
		...
		break;
	default:
		wpa_printf(0, "EAP-PAX: ignoring message with unknown "
...
static int eap_pax_isKeyAvailable(struct eap_sm *sm, void *priv)
{
	struct eap_pax_data *data = priv;
...
static unsigned char * eap_pax_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_pax_data *data = priv;
	unsigned char *key;
	...
	*len = 64;
	eap_pax_kdf(data->mac_id, data->mk, 16,
		    "Master Session Key", data->rand.e, 2 * 32,
		    64, key);
...
static unsigned char * eap_pax_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_pax_data *data = priv;
	unsigned char *key;
