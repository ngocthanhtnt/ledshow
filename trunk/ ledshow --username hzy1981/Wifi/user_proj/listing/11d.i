#line 1 "..\\marvel\\driver\\src\\11d.c"
...
#line 5 "..\\marvel\\driver\\src\\11d.c"
...
#line 6 "..\\marvel\\driver\\src\\11d.c"
#line 7 "..\\marvel\\driver\\src\\11d.c"
#line 8 "..\\marvel\\driver\\src\\11d.c"
#line 1 "..\\marvel\\driver\\inc\\host_cmd.h"
...
#line 92 "..\\marvel\\driver\\inc\\host_cmd.h"
...
#line 148 "..\\marvel\\driver\\inc\\host_cmd.h"

 
#line 156 "..\\marvel\\driver\\inc\\host_cmd.h"
...
#line 176 "..\\marvel\\driver\\inc\\host_cmd.h"
...
#line 199 "..\\marvel\\driver\\inc\\host_cmd.h"
...
#line 304 "..\\marvel\\driver\\inc\\host_cmd.h"



#line 9 "..\\marvel\\driver\\src\\11d.c"
...
#line 10 "..\\marvel\\driver\\src\\11d.c"
...
static u8 lbs_region_2_code(u8 *region)
{
	...
	for (i = 0; i < 3 && region[i]; i++)
		region[i] = toupper(region[i]);

	for (i = 0; i < (sizeof(region_code_mapping) / sizeof((region_code_mapping)[0])); i++) {
 ...
 
{
	...
	cfp_no = (sizeof(channel_freq_power_UN_BG) / sizeof((channel_freq_power_UN_BG)[0]));
			  ...
			  struct parsed_region_chan_11d * parsed_region_chan)
{
	...
	lbs_hex(0x00000400, "parsed_region_chan", (u8 *)chanpwr,
...
u32 lbs_chan_2_freq(u8 chan)
{
	...
	for (i = 0; i < (sizeof(channel_freq_power_UN_BG) / sizeof((channel_freq_power_UN_BG)[0])); i++) {
				  ...
				  struct lbs_802_11d_domain_reg *domaininfo)
{
	...
	;
	lbs_hex(0x00000400, "parsed_region_chan", (u8 *)parsed_region_chan,
	...
	;
	lbs_hex(0x00000400, "domaininfo", (u8 *)domaininfo,
		3 + 1 +
					  ...
					  parsed_region_chan)
{
	...
	memcpy(parsed_region_chan->countrycode,
	       lbs_code_2_region(region_chan->region), 3);
				 ...
				 struct parsed_region_chan_11d *parsed_region_chan)
{
	...
	lbs_hex(0x00000400, "countryinfo", (u8 *) countryinfo, 30);

	if ((*(countryinfo->countrycode)) == 0
	    || (countryinfo->header.len <= 3)) {
	...
	;
	lbs_hex(0x00000400, "countrycode", (u8 *)countryinfo->countrycode,
		3);
	...
	nr_subband = (countryinfo->header.len - 3) /
	...
	;
	lbs_hex(0x00000400, "parsed_region_chan", (u8 *) parsed_region_chan,
		2 + 3 + sizeof(struct parsed_region_chan_11d) * idx);
...
static int set_domain_info_11d(struct lbs_private *priv)
{
	...
	ret = lbs_prepare_and_send_command(priv, 0x005b,
				    0x0001,
				    0x0002, 0, 0);
	if (ret)
		;
...
int lbs_set_universaltable(struct lbs_private *priv, u8 band)
{
	...
	priv->universal_channel[i].region = 0xff;
				 ...
				 u16 cmdoption)
{
	...
	if (cmdoption == 0x0000) {
		cmd->size =
		    (sizeof(pdomaininfo->action) + sizeof(struct cmd_ds_gen));
		lbs_hex(0x00000400, "802_11D_DOMAIN_INFO", (u8 *) cmd,
			(cmd->size));
		goto done;
	}

	domain->header.type = (0x0007);
	memcpy(domain->countrycode, priv->domainreg.countrycode,
	       sizeof(domain->countrycode));

	domain->header.len =
	    (nr_subband * sizeof(struct ieee_subbandset) + sizeof(domain->countrycode));
	...
	if (nr_subband) {
		...
		cmd->size = (sizeof(pdomaininfo->action) + (domain->header . len) + sizeof(struct mrvl_ie_header) + sizeof(struct cmd_ds_gen));
	...
	lbs_hex(0x00000400, "802_11D_DOMAIN_INFO", (u8 *) cmd, (cmd->size));
...
int lbs_ret_802_11d_domain_info(struct cmd_ds_command *resp)
{
	...
	u16 action = (domaininfo->action);
	s16 ret = 0;
	u8 nr_subband = 0;

	;

	lbs_hex(0x00000400, "domain info resp", (u8 *) resp,
		(int)(resp->size));

	nr_subband = ((domain->header . len) - 3) /
		      sizeof(struct ieee_subbandset);
...
int lbs_create_dnld_countryinfo_11d(struct lbs_private *priv)
{
	...
	if (priv->enable11d) {
		...
		for (j = 0; j < (sizeof(priv->region_channel) / sizeof((priv->region_channel)[0])); j++) {
		...
		if (j >= (sizeof(priv->region_channel) / sizeof((priv->region_channel)[0]))) {
			;
