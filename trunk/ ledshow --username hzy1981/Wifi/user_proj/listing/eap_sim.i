#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 21 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 22 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"
...
#line 23 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"


struct eap_sim_data {
	unsigned char *ver_list;
	size_t ver_list_len;
	int selected_version;
	size_t min_num_chal, num_chal;

	unsigned char kc[3][8];
	unsigned char sres[3][4];
	unsigned char nonce_mt[16], nonce_s[16];
	unsigned char mk[20];
	unsigned char k_aut[16];
	unsigned char k_encr[16];
	unsigned char msk[64];
	unsigned char emsk[64];
	unsigned char rand[3][16];
...
static void * eap_sim_init(struct eap_sm *sm)
{
	...
	if (os_get_random(data->nonce_mt, 16)) {
		wpa_printf(0, "EAP-SIM: Failed to get random data "
			   "for NONCE_MT");
		os_free(data);
		return 0;
	}
	...
	if (config && config->phase1) {
		...
		if (pos) {
			...
			wpa_printf(0, "EAP-SIM: Set minimum number of "
				   "challenges to %lu",
				   (unsigned long) data->min_num_chal);
		}
...
static int eap_sim_gsm_auth(struct eap_sm *sm, struct eap_sim_data *data)
{
	...
	wpa_printf(0, "EAP-SIM: GSM authentication algorithm");
...
#line 213 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"

#line 251 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sim.c"

	wpa_printf(0, "EAP-SIM: No GSM authentication algorithm "
		   "enabled");
	return -1;
...
{
	wpa_printf(0, "EAP-SIM: forgetting old%s%s%s",
		   id & 0x01 ? " pseudonym" : "",
		   id & 0x02 ? " reauth_id" : "",
		   id & 0x04 ? " eap_id" : "");
	if (id & 0x01) {
			     ...
			     struct eap_sim_attrs *attr)
{
	if (attr->next_pseudonym) {
		...
		if (data->pseudonym == 0) {
			wpa_printf(0, "EAP-SIM: (encr) No memory for "
				   "next pseudonym");
			return -1;
		}
...
static struct wpabuf * eap_sim_client_error(struct eap_sim_data *data, unsigned char id,
					    int err)
{
...
static struct wpabuf * eap_sim_response_start(struct eap_sm *sm,
					      struct eap_sim_data *data, unsigned char id,
					      enum eap_sim_id_req id_req)
{
	...
	if (id_req == ANY_ID && data->reauth_id) {
		...
		eap_sim_clear_identities(data, 0x02);
	} else if (id_req != NO_ID_REQ) {
		identity = eap_get_config_identity(sm, &identity_len);
		if (identity) {
			eap_sim_clear_identities(data, 0x01 |
						 0x02);
		}
	}
	if (id_req != NO_ID_REQ)
		eap_sim_clear_identities(data, 0x04);

	wpa_printf(0, "Generating EAP-SIM Start (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id,
			       EAP_TYPE_SIM, 10);
	if (!data->reauth) {
		wpa_hexdump(0, "   AT_NONCE_MT",
			    data->nonce_mt, 16);
		eap_sim_msg_add(msg, 7, 0,
				data->nonce_mt, 16);
		wpa_printf(0, "   AT_SELECTED_VERSION %d",
			   data->selected_version);
		eap_sim_msg_add(msg, 16,
				data->selected_version, 0, 0);
	}
						  ...
						  unsigned char id)
{
	...
	wpa_printf(0, "Generating EAP-SIM Challenge (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, EAP_TYPE_SIM,
			       11);
	...
	return eap_sim_msg_finish(msg, data->k_aut, (unsigned char *) data->sres,
				  data->num_chal * 4);
}
					       ...
					       unsigned char id, int counter_too_small)
{
	...
	wpa_printf(0, "Generating EAP-SIM Reauthentication (id=%d)",
		   id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, EAP_TYPE_SIM,
			       13);
	wpa_printf(0, "   AT_IV");
	...
	if (eap_sim_msg_add_encr_end(msg, data->k_encr, 6)) {
		wpa_printf(0, "EAP-SIM: Failed to encrypt "
			   "AT_ENCR_DATA");
						     ...
						     unsigned char id, unsigned short notification)
{
	...
	wpa_printf(0, "Generating EAP-SIM Notification (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id,
			       EAP_TYPE_SIM, 12);
					     ...
					     struct eap_sim_attrs *attr)
{
	...
	wpa_printf(0, "EAP-SIM: subtype Start");
	if (attr->version_list == 0) {
		wpa_printf(0, "EAP-SIM: No AT_VERSION_LIST in "
			   "SIM/Start");
	...
	if (data->ver_list == 0) {
		wpa_printf(0, "EAP-SIM: Failed to allocate "
	...
	if (selected_version < 0) {
		wpa_printf(0, "EAP-SIM: Could not find a supported "
			   "version");
		return eap_sim_client_error(data, id,
					    1);
	}
	wpa_printf(0, "EAP-SIM: Selected Version %d",
		   selected_version);
	...
	if (id_error) {
		wpa_printf(0, "EAP-SIM: Too many ID requests "
						 ...
						 struct eap_sim_attrs *attr)
{
	...
	wpa_printf(0, "EAP-SIM: subtype Challenge");
	data->reauth = 0;
	if (!attr->mac || !attr->rand) {
		wpa_printf(0, "EAP-SIM: Challenge message "
			   "did not include%s%s",
			   !attr->mac ? " AT_MAC" : "",
			   !attr->rand ? " AT_RAND" : "");
		return eap_sim_client_error(data, id,
	...
	wpa_printf(0, "EAP-SIM: %lu challenges",
		   (unsigned long) attr->num_chal);
	if (attr->num_chal < data->min_num_chal) {
		wpa_printf(0, "EAP-SIM: Insufficient number of "
			   "challenges (%lu)", (unsigned long) attr->num_chal);
		return eap_sim_client_error(data, id,
					    2);
	}
	if (attr->num_chal > 3) {
		wpa_printf(0, "EAP-SIM: Too many challenges "
	...
	if (os_memcmp(attr->rand, attr->rand + 16,
		   16) == 0 ||
	    (attr->num_chal > 2 &&
	     (os_memcmp(attr->rand, attr->rand + 2 * 16,
			16) == 0 ||
	      os_memcmp(attr->rand + 16,
			attr->rand + 2 * 16,
			16) == 0))) {
		wpa_printf(0, "EAP-SIM: Same RAND used multiple times");
		return eap_sim_client_error(data, id,
					    3);
	}

	os_memcpy(data->rand, attr->rand, attr->num_chal * 16);
	data->num_chal = attr->num_chal;
		
	if (eap_sim_gsm_auth(sm, data)) {
		wpa_printf(0, "EAP-SIM: GSM authentication failed");
		return eap_sim_client_error(data, id,
					    0);
	}
	if (data->last_eap_identity) {
		...
		identity = eap_get_config_identity(sm, &identity_len);
	wpa_hexdump_ascii(0, "EAP-SIM: Selected identity for MK "
	...
	eap_sim_derive_mk(identity, identity_len, data->nonce_mt,
			  ...
			  (const unsigned char *) data->kc, data->mk);
	...
	eap_sim_clear_identities(data, 0x01 | 0x02 |
				 0x04);
					       ...
					       struct eap_sim_attrs *attr)
{
	...
	if (attr->encr_data == 0 || attr->iv == 0) {
		wpa_printf(0, "EAP-SIM: Notification message after "
	...
	decrypted = eap_sim_parse_encr(data->k_encr, attr->encr_data,
				       ...
				       0);
	if (decrypted == 0) {
		wpa_printf(0, "EAP-SIM: Failed to parse encrypted "
			   "data from notification message");
		return -1;
	}

	if (eattr.counter < 0 || (size_t) eattr.counter != data->counter) {
		wpa_printf(0, "EAP-SIM: Counter in notification "
...
{
	if (attr->mac == 0) {
		wpa_printf(0, "EAP-SIM: no AT_MAC in after_auth "
			   "Notification message");
		return -1;
	}

	if (eap_sim_verify_mac(data->k_aut, reqData, attr->mac, (unsigned char *) "", 0))
	{
		wpa_printf(0, "EAP-SIM: Notification message "
			   "used invalid AT_MAC");
		return -1;
	}
	...
	if (data->reauth &&
		...
		wpa_printf(0, "EAP-SIM: Invalid notification "
...
static struct wpabuf * eap_sim_process_notification(
	struct eap_sm *sm, struct eap_sim_data *data, unsigned char id,
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	wpa_printf(0, "EAP-SIM: subtype Notification");
	if (data->num_notification > 0) {
		wpa_printf(0, "EAP-SIM: too many notification "
	...
	if (attr->notification == -1) {
		wpa_printf(0, "EAP-SIM: no AT_NOTIFICATION in "
			   "Notification message");
		return eap_sim_client_error(data, id,
					    0);
	}
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	wpa_printf(0, "EAP-SIM: subtype Reauthentication");

	if (data->reauth_id == 0) {
		wpa_printf(0, "EAP-SIM: Server is trying "
	...
	if (eap_sim_verify_mac(data->k_aut, reqData, attr->mac, (unsigned char *) "", 0))
	{
		wpa_printf(0, "EAP-SIM: Reauthentication "
	...
	if (eattr.nonce_s == 0 || eattr.counter < 0) {
		wpa_printf(0, "EAP-SIM: (encr) No%s%s in reauth packet",
	...
	if (eattr.counter < 0 || (size_t) eattr.counter <= data->counter) {
		wpa_printf(0, "EAP-SIM: (encr) Invalid counter "
	...
	wpa_hexdump(0, "EAP-SIM: (encr) AT_NONCE_S",
		    data->nonce_s, 16);
	...
	eap_sim_derive_keys_reauth(data->counter,
				   ...
				   data->emsk);
	eap_sim_clear_identities(data, 0x02 | 0x04);
	eap_sim_learn_ids(data, &eattr);
	...
	if (data->counter > 1000) {
		wpa_printf(0, "EAP-SIM: Maximum number of "
				       ...
				       const struct wpabuf *reqData)
{
	...
	wpa_hexdump_buf(0, "EAP-SIM: EAP data", reqData);
	if (eap_get_config_identity(sm, &len) == 0) {
		wpa_printf(0, "EAP-SIM: Identity not configured");
	...
	subtype = *pos++;
	wpa_printf(0, "EAP-SIM: Subtype=%d", subtype);
	pos += 2;  
	...
	case 13:
		...
		break;
	case 14:
		wpa_printf(0, "EAP-SIM: subtype Client-Error");
		res = eap_sim_client_error(data, id,
					   0);
		break;
	default:
		wpa_printf(0, "EAP-SIM: Unknown subtype=%d", subtype);
		res = eap_sim_client_error(data, id,
					   0);
		break;
	}
...
static int eap_sim_has_reauth_data(struct eap_sm *sm, void *priv)
{
	struct eap_sim_data *data = priv;
...
static const unsigned char * eap_sim_get_identity(struct eap_sm *sm, void *priv,
				       size_t *len)
{
	struct eap_sim_data *data = priv;
...
static int eap_sim_isKeyAvailable(struct eap_sm *sm, void *priv)
{
	struct eap_sim_data *data = priv;
...
static unsigned char * eap_sim_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_sim_data *data = priv;
	unsigned char *key;
...
static unsigned char * eap_sim_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_sim_data *data = priv;
	unsigned char *key;
