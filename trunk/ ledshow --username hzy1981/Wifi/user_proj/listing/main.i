...
static int dns_test_test()
{
	...
	struct ip_addr addr;
	err_t err;
	if (err = dns_gethostbyname((char*)(hostname), &(addr),dns_found_callbackx,0) == 0) {
	} else {
	}
}
...
#line 77 "..\\main\\main.c"
	...
	NVIC_Configuration();
	GPIO_Configuration();
	USART_Configuration(((USART_TypeDef *) ((((u32)0x40000000) + 0x10000) + 0x3800)));
	...
	while(1){
		if((priv->connect_status==LBS_DISCONNECTED)&&marvel_mode=='0')
		{
			printf("LBS_DISCONNECTED!!!\r\n");
#line 150 "..\\main\\main.c"
			while(1);
		...
		if(app_time>10){ 
			if (newpower_time_out_current_state != 8  && marvel_mode == 0)  
				continue;
