#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_pax_common.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_pax_common.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_pax_common.c"
...
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_pax_common.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common\\eap_pax_common.c"
		...
		size_t output_len, unsigned char *output)
{
	...
	unsigned char counter, *pos;
	const unsigned char *addr[3];
	size_t len[3];
	size_t num_blocks, left;

	num_blocks = (output_len + 16 - 1) / 16;
	if (identifier == 0 || num_blocks >= 255)
		return -1;
	...
	addr[0] = (const unsigned char *) identifier;
	...
	pos = output;
	left = output_len;
	for (counter = 1; counter <= (unsigned char) num_blocks; counter++) {
		size_t clen = left > 16 ? 16 : left;
...
int eap_pax_initial_key_derivation(unsigned char mac_id, const unsigned char *ak, const unsigned char *e,
				   unsigned char *mk, unsigned char *ck, unsigned char *ick)
{
	wpa_printf(0, "EAP-PAX: initial key derivation");
	if (eap_pax_kdf(mac_id, ak, 16, "Master Key",
			e, 2 * 32, 16, mk) ||
	    eap_pax_kdf(mac_id, mk, 16, "Confirmation Key",
			e, 2 * 32, 16, ck) ||
	    eap_pax_kdf(mac_id, mk, 16, "Integrity Check Key",
			e, 2 * 32, 16, ick))
		return -1;

	wpa_hexdump_key(0, "EAP-PAX: AK", ak, 16);
	wpa_hexdump_key(0, "EAP-PAX: MK", mk, 16);
	wpa_hexdump_key(0, "EAP-PAX: CK", ck, 16);
	wpa_hexdump_key(0, "EAP-PAX: ICK", ick, 16);
