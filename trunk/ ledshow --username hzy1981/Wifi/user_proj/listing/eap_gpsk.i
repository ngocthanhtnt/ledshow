#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_gpsk.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_gpsk.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_gpsk.c"
...
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_gpsk.c"
#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common/eap_gpsk_common.h"
...
#line 24 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_common/eap_gpsk_common.h"
...
int eap_gpsk_derive_keys(const unsigned char *psk, size_t psk_len, int vendor,
			 int specifier,
			 const unsigned char *rand_client, const unsigned char *rand_server,
			 const unsigned char *id_client, size_t id_client_len,
			 const unsigned char *id_server, size_t id_server_len,
			 unsigned char *msk, unsigned char *emsk, unsigned char *sk, size_t *sk_len,
			 unsigned char *pk, size_t *pk_len);
size_t eap_gpsk_mic_len(int vendor, int specifier);
int eap_gpsk_compute_mic(const unsigned char *sk, size_t sk_len, int vendor,
			 int specifier, const unsigned char *data, size_t len, unsigned char *mic);

#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_gpsk.c"

struct eap_gpsk_data {
	enum { GPSK_1, GPSK_3, SUCCESS, FAILURE } state;
	unsigned char rand_server[32];
	unsigned char rand_peer[32];
...
static struct wpabuf * eap_gpsk_send_gpsk_2(struct eap_gpsk_data *data,
					    unsigned char identifier,
					    const unsigned char *csuite_list,
...
static void * eap_gpsk_init(struct eap_sm *sm)
{
	...
	if (password == 0) {
		wpa_printf(0, "EAP-GPSK: No key (password) configured");
		return 0;
	}
...
static const unsigned char * eap_gpsk_process_id_server(struct eap_gpsk_data *data,
					     const unsigned char *pos, const unsigned char *end)
{
	unsigned short alen;

	if (end - pos < 2) {
		wpa_printf(0, "EAP-GPSK: Too short GPSK-1 packet");
		return 0;
	}
	alen = ((unsigned short) (((pos)[0] << 8) | (pos)[1]));
	pos += 2;
	if (end - pos < alen) {
		wpa_printf(0, "EAP-GPSK: ID_Server overflow");
		return 0;
	}
	os_free(data->id_server);
	data->id_server = os_malloc(alen);
	if (data->id_server == 0) {
		wpa_printf(0, "EAP-GPSK: No memory for ID_Server");
		return 0;
	}
	os_memcpy(data->id_server, pos, alen);
	data->id_server_len = alen;
	wpa_hexdump_ascii(0, "EAP-GPSK: ID_Server",
...
static const unsigned char * eap_gpsk_process_rand_server(struct eap_gpsk_data *data,
					       const unsigned char *pos, const unsigned char *end)
{
	if (pos == 0)
		return 0;
	...
	if (end - pos < 32) {
		wpa_printf(0, "EAP-GPSK: RAND_Server overflow");
		return 0;
	}
	os_memcpy(data->rand_server, pos, 32);
	wpa_hexdump(0, "EAP-GPSK: RAND_Server",
		    data->rand_server, 32);
	pos += 32;
				  ...
				  size_t csuite_list_len)
{
	...
	data->vendor = 0x00000000;
	data->specifier = 0x000000;
	csuite = (struct eap_gpsk_csuite *) csuite_list;
	for (i = 0; i < count; i++) {
		int vendor, specifier;
		vendor = ((((unsigned int) (csuite ->vendor)[0]) << 24) | (((unsigned int) (csuite ->vendor)[1]) << 16) | (((unsigned int) (csuite ->vendor)[2]) << 8) | ((unsigned int) (csuite ->vendor)[3]));
		specifier = ((unsigned short) (((csuite ->specifier)[0] << 8) | (csuite ->specifier)[1]));
		wpa_printf(0, "EAP-GPSK: CSuite[%d]: %d:%d",
			   i, vendor, specifier);
		if (data->vendor == 0x00000000 &&
		    data->specifier == 0x000000 &&
	...
	if (data->vendor == 0x00000000 &&
	    data->specifier == 0x000000) {
		wpa_msg(sm->msg_ctx, 0, "EAP-GPSK: No supported "
			"ciphersuite found");
		return -1;
	}
	wpa_printf(0, "EAP-GPSK: Selected ciphersuite %d:%d",
...
static const unsigned char * eap_gpsk_process_csuite_list(struct eap_sm *sm,
					       struct eap_gpsk_data *data,
					       ...
					       const unsigned char *pos, const unsigned char *end)
{
	...
	*list_len = ((unsigned short) (((pos)[0] << 8) | (pos)[1]));
	pos += 2;
	if (end - pos < (int) *list_len) {
		wpa_printf(0, "EAP-GPSK: CSuite_List overflow");
		return 0;
	}
	if (*list_len == 0 || (*list_len % sizeof(struct eap_gpsk_csuite))) {
		wpa_printf(0, "EAP-GPSK: Invalid CSuite_List len %lu",
...
{
	size_t csuite_list_len;
	const unsigned char *csuite_list, *pos, *end;
	struct wpabuf *resp;
	...
	wpa_printf(0, "EAP-GPSK: Received Request/GPSK-1");
					    ...
					    size_t csuite_list_len)
{
	struct wpabuf *resp;
	size_t len, miclen;
	unsigned char *rpos, *start;
	struct eap_gpsk_csuite *csuite;

	wpa_printf(0, "EAP-GPSK: Sending Response/GPSK-2");

	miclen = eap_gpsk_mic_len(data->vendor, data->specifier);
	len = 1 + 2 + data->id_peer_len + 2 + data->id_server_len +
		2 * 32 + 2 + csuite_list_len +
	...
	wpa_hexdump_ascii(0, "EAP-GPSK: ID_Peer",
	...
	if (os_get_random(data->rand_peer, 32)) {
		wpa_printf(0, "EAP-GPSK: Failed to get random data "
			   "for RAND_Peer");
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return 0;
	}
	wpa_hexdump(0, "EAP-GPSK: RAND_Peer",
		    data->rand_peer, 32);
	wpabuf_put_data(resp, data->rand_peer, 32);
	wpabuf_put_data(resp, data->rand_server, 32);

	wpabuf_put_be16(resp, csuite_list_len);
	wpabuf_put_data(resp, csuite_list, csuite_list_len);

	csuite = wpabuf_put(resp, sizeof(*csuite));
	do { (csuite ->vendor)[0] = (unsigned char) ((((unsigned int) (data->vendor)) >> 24) & 0xff); (csuite ->vendor)[1] = (unsigned char) ((((unsigned int) (data->vendor)) >> 16) & 0xff); (csuite ->vendor)[2] = (unsigned char) ((((unsigned int) (data->vendor)) >> 8) & 0xff); (csuite ->vendor)[3] = (unsigned char) (((unsigned int) (data->vendor)) & 0xff); } while (0);
	do { (csuite ->specifier)[0] = ((unsigned short) (data->specifier)) >> 8; (csuite ->specifier)[1] = ((unsigned short) (data->specifier)) & 0xff; } while (0);
	...
	if (eap_gpsk_derive_keys(data->psk, data->psk_len,
		...
		wpa_printf(0, "EAP-GPSK: Failed to derive keys");
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return 0;
	}
...
static const unsigned char * eap_gpsk_validate_rand(struct eap_gpsk_data *data,
					 const unsigned char *pos, const unsigned char *end)
{
	if (end - pos < 32) {
		wpa_printf(0, "EAP-GPSK: Message too short for "
			   "RAND_Peer");
		return 0;
	}
	if (os_memcmp(pos, data->rand_peer, 32) != 0) {
		wpa_printf(0, "EAP-GPSK: RAND_Peer in GPSK-2 and "
			   "GPSK-3 did not match");
		wpa_hexdump(0, "EAP-GPSK: RAND_Peer in GPSK-2",
			    data->rand_peer, 32);
		wpa_hexdump(0, "EAP-GPSK: RAND_Peer in GPSK-3",
			    pos, 32);
		return 0;
	}
	pos += 32;
	...
	if (os_memcmp(pos, data->rand_server, 32) != 0) {
		wpa_printf(0, "EAP-GPSK: RAND_Server in GPSK-1 and "
			   "GPSK-3 did not match");
		wpa_hexdump(0, "EAP-GPSK: RAND_Server in GPSK-1",
			    data->rand_server, 32);
		wpa_hexdump(0, "EAP-GPSK: RAND_Server in GPSK-3",
			    pos, 32);
		return 0;
	}
	pos += 32;
...
static const unsigned char * eap_gpsk_validate_id_server(struct eap_gpsk_data *data,
					      const unsigned char *pos, const unsigned char *end)
{
	size_t len;
	...
	if (len != data->id_server_len ||
		...
		wpa_printf(0, "EAP-GPSK: ID_Server did not match with "
			   "the one used in GPSK-1");
		wpa_hexdump_ascii(0, "EAP-GPSK: ID_Server in GPSK-1",
				  data->id_server, data->id_server_len);
		wpa_hexdump_ascii(0, "EAP-GPSK: ID_Server in GPSK-3",
				  pos, len);
		return 0;
	}
...
static const unsigned char * eap_gpsk_validate_csuite(struct eap_gpsk_data *data,
					   const unsigned char *pos, const unsigned char *end)
{
	int vendor, specifier;
	...
	if (vendor != data->vendor || specifier != data->specifier) {
		wpa_printf(0, "EAP-GPSK: CSuite_Sel (%d:%d) does not "
...
static const unsigned char * eap_gpsk_validate_pd_payload_2(struct eap_gpsk_data *data,
						 const unsigned char *pos, const unsigned char *end)
{
	unsigned short alen;
	...
	wpa_hexdump(0, "EAP-GPSK: PD_Payload_2", pos, alen);
	pos += alen;
...
static const unsigned char * eap_gpsk_validate_gpsk_3_mic(struct eap_gpsk_data *data,
					       const unsigned char *payload,
					       const unsigned char *pos, const unsigned char *end)
{
	size_t miclen;
	...
	if (end - pos < (int) miclen) {
		wpa_printf(0, "EAP-GPSK: Message too short for MIC "
			   "(left=%lu miclen=%lu)",
	...
	if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
	    ...
	    < 0) {
		wpa_printf(0, "EAP-GPSK: Failed to compute MIC");
		return 0;
	}
	if (os_memcmp(mic, pos, miclen) != 0) {
		wpa_printf(0, "EAP-GPSK: Incorrect MIC in GPSK-3");
		wpa_hexdump(0, "EAP-GPSK: Received MIC", pos, miclen);
		wpa_hexdump(0, "EAP-GPSK: Computed MIC", mic, miclen);
		return 0;
	}
	pos += miclen;
					       ...
					       size_t payload_len)
{
	...
	wpa_printf(0, "EAP-GPSK: Received Request/GPSK-3");
	...
	if (pos != end) {
		wpa_printf(0, "EAP-GPSK: Ignored %lu bytes of extra "
					    ...
					    unsigned char identifier)
{
	...
	wpa_printf(0, "EAP-GPSK: Sending Response/GPSK-4");
					...
					const struct wpabuf *reqData)
{
	...
	wpa_printf(0, "EAP-GPSK: Received frame: opcode %d", *pos);
	...
	case 3:
		...
		break;
	default:
		wpa_printf(0, "EAP-GPSK: Ignoring message with "
...
static int eap_gpsk_isKeyAvailable(struct eap_sm *sm, void *priv)
{
	struct eap_gpsk_data *data = priv;
...
static unsigned char * eap_gpsk_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_gpsk_data *data = priv;
	unsigned char *key;
...
static unsigned char * eap_gpsk_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_gpsk_data *data = priv;
	unsigned char *key;
