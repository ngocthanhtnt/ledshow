#line 1 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sake.c"
...
#line 16 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sake.c"
...
#line 18 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sake.c"
...
#line 19 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sake.c"
...
#line 20 "..\\wpa_supplicant-0.7.3\\wpa_supplicant-0.7.3\\src\\eap_peer\\eap_sake.c"

struct eap_sake_data {
	enum { IDENTITY, CHALLENGE, CONFIRM, SUCCESS, FAILURE } state;
	unsigned char root_secret_a[16];
	unsigned char root_secret_b[16];
	unsigned char rand_s[16];
	unsigned char rand_p[16];
	struct {
...
static void * eap_sake_init(struct eap_sm *sm)
{
	...
	if (!password || password_len != 2 * 16) {
		wpa_printf(0, "EAP-SAKE: No key of correct length "
			   "configured");
		return 0;
	}
	...
	os_memcpy(data->root_secret_a, password, 16);
...
static struct wpabuf * eap_sake_build_msg(struct eap_sake_data *data,
					  int id, size_t length, unsigned char subtype)
{
	...
	if (msg == 0) {
		wpa_printf(0, "EAP-SAKE: Failed to allocate memory "
			   "request");
		return 0;
	}
						 ...
						 size_t payload_len)
{
	...
	wpa_printf(0, "EAP-SAKE: Received Request/Identity");

	if (eap_sake_parse_attributes(payload, payload_len, &attr))
		return 0;

	if (!attr.perm_id_req && !attr.any_id_req) {
		wpa_printf(0, "EAP-SAKE: No AT_PERM_ID_REQ or "
			   "AT_ANY_ID_REQ in Request/Identity");
		return 0;
	}

	wpa_printf(0, "EAP-SAKE: Sending Response/Identity");
	...
	wpa_printf(0, "EAP-SAKE: * AT_PEERID");
	eap_sake_add_attr(resp, 6,
						  ...
						  size_t payload_len)
{
	...
	if (data->state != IDENTITY && data->state != CHALLENGE) {
		wpa_printf(0, "EAP-SAKE: Request/Challenge received "
	...
	wpa_printf(0, "EAP-SAKE: Received Request/Challenge");

	if (eap_sake_parse_attributes(payload, payload_len, &attr))
		return 0;

	if (!attr.rand_s) {
		wpa_printf(0, "EAP-SAKE: Request/Challenge did not "
			   "include AT_RAND_S");
		return 0;
	}

	os_memcpy(data->rand_s, attr.rand_s, 16);
	wpa_hexdump(0, "EAP-SAKE: RAND_S (server rand)",
		    data->rand_s, 16);

	if (os_get_random(data->rand_p, 16)) {
		wpa_printf(0, "EAP-SAKE: Failed to get random data");
		return 0;
	}
	wpa_hexdump(0, "EAP-SAKE: RAND_P (peer rand)",
		    data->rand_p, 16);
	...
	data->serverid_len = 0;
	if (attr.serverid) {
		wpa_hexdump_ascii(0, "EAP-SAKE: SERVERID",
	...
	eap_sake_derive_keys(data->root_secret_a, data->root_secret_b,
			     ...
			     (unsigned char *) &data->tek, data->msk, data->emsk);

	wpa_printf(0, "EAP-SAKE: Sending Response/Challenge");
	...
	wpa_printf(0, "EAP-SAKE: * AT_RAND_P");
	eap_sake_add_attr(resp, 2,
			  data->rand_p, 16);
	...
	wpa_printf(0, "EAP-SAKE: * AT_MIC_P");
	wpabuf_put_u8(resp, 4);
	wpabuf_put_u8(resp, 2 + 16);
	...
	if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
				 ...
				 rpos)) {
		wpa_printf(0, "EAP-SAKE: Failed to compute MIC");
		wpabuf_free(resp);
		return 0;
	}
						...
						size_t payload_len)
{
	...
	wpa_printf(0, "EAP-SAKE: Received Request/Confirm");

	if (eap_sake_parse_attributes(payload, payload_len, &attr))
		return 0;

	if (!attr.mic_s) {
		wpa_printf(0, "EAP-SAKE: Request/Confirm did not "
			   "include AT_MIC_S");
		return 0;
	}
	...
	eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
			     ...
			     attr.mic_s, mic_s);
	if (os_memcmp(attr.mic_s, mic_s, 16) != 0) {
		wpa_printf(0, "EAP-SAKE: Incorrect AT_MIC_S");
		eap_sake_state(data, FAILURE);
		ret->methodState = METHOD_DONE;
		ret->decision = DECISION_FAIL;
		ret->allowNotifications = 0;
		wpa_printf(0, "EAP-SAKE: Sending "
			   "Response/Auth-Reject");
		return eap_sake_build_msg(data, eap_get_id(reqData), 0,
					  3);
	}

	wpa_printf(0, "EAP-SAKE: Sending Response/Confirm");
					...
					const struct wpabuf *reqData)
{
	...
	size_t len;
	unsigned char subtype, session_id;

	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_SAKE, reqData, &len);
	if (pos == 0 || len < sizeof(struct eap_sake_hdr)) {
		ret->ignore = 1;
		return 0;
	}
	...
	wpa_printf(0, "EAP-SAKE: Received frame: subtype %d "
		   "session_id %d", subtype, session_id);
	wpa_hexdump(0, "EAP-SAKE: Received attributes",
		    pos, end - pos);

	if (data->session_id_set && data->session_id != session_id) {
		wpa_printf(0, "EAP-SAKE: Session ID mismatch (%d,%d)",
	...
	case 2:
		...
		break;
	default:
		wpa_printf(0, "EAP-SAKE: Ignoring message with "
...
static int eap_sake_isKeyAvailable(struct eap_sm *sm, void *priv)
{
	struct eap_sake_data *data = priv;
...
static unsigned char * eap_sake_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_sake_data *data = priv;
	unsigned char *key;
...
static unsigned char * eap_sake_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
	struct eap_sake_data *data = priv;
	unsigned char *key;
