...
u8 enc28j60ReadOp(u8 op, u8 address)		   //RCR  000
{
   u8 dat = 0;
...
void enc28j60WriteOp(u8 op, u8 address, u8 data)	   //WCR  010
{
   u8 dat = 0;
...
void enc28j60ReadBuffer(u16 len, u8* data)		    //RBM  001
{
   ENC28J60_CSL();
   // issue read command
   SPInet_ReadWrite(ENC28J60_READ_BUF_MEM);  	//	RBM操作码为001  缓存区地址为 1AH   即为3AH
   while (len--)
   {
...
void enc28j60WriteBuffer(u16 len, u8* data)		  //WBM  011
{
   ENC28J60_CSL();
   // issue write command
...
void enc28j60PhyWrite(u8 address, u16 data)  //写PHY寄存器
{
