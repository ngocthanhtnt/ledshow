...
//#undef SCAN_INT_DISABLE()
//#define SCAN_INT_DISABLE()

//#undef SCAN_INT_ENABLE()
  ...
  S_Show_Data *pDst_Buf, S_Point *pPoint1, INT8U Flag)
{
    ...
    INT16U Len, Diff,Mask,Mask1,X0,X1,Y0,Y1,Width,Height,WidthBytes;//,Y1;
    INT8U *pSrc, *pDst, Data, Color_Num, *pSrc0, *pDst0;
    INT16S i,j;
/*
      for(i = 0; i < X_Len;  i++)
        for(j = 0; j < Y_Len; j++)
...
*/
    ...
    WidthBytes = Screen_Para.Base_Para.Width / 8 * Color_Num; //一行的字节数
    ...
    Mask1 = Bit_Mask[((X1 + X_Len - 1) & 0x07) + 1];

    if(pSrc_Buf != pDst_Buf || (X0 >= X1 && Y0 >= Y1)) //如果是不同显示缓冲间拷贝或者同一缓冲区源在目标右下角，则从左向右，从上向下复制
    {
    if((X0 % 8) >= (X1 % 8)) //源数据需要左移
    {
        for(i = 0; i < Len + 2; i ++)
        {
            ...
            pSrc = pSrc0 + j * WidthBytes + i * Color_Num;//pSrc_Buf->Color_Data + (Index0 >> 3) * Color_Num;
            pDst = pDst0 + j * WidthBytes + i * Color_Num;//pDst_Buf->Color_Data + (Index1 >> 3) * Color_Num;
    ...
    else
    {
        for(i = 0; i < Len + 2; i ++)
        {
            ...
            for(j = 0; j < Y_Len; j ++)
            {
                ...
                else if(i < Len + 1)//for(j = 0; j < Len; j ++)
                {
   ...
   else if(X0 <= X1 && Y0 >= Y1) //源在目标左下，则从右向做，从上向下复制
    {
        if((X0 % 8) >= (X1 % 8)) //源数据需要左移
        {
            for(i = Len + 1; i >= 0; i --)
   ...
   else if(X0 >= X1 && Y0 < Y1) //如果是不同显示缓冲间拷贝或者同一缓冲区源在目标右上角，则从左向右，从下向上复制
   {
        if((X0 % 8) >= (X1 % 8)) //源数据需要左移
        {
            for(i = 0; i < Len + 2; i ++)
   ...
   else if(X0 <= X1 && Y0 < Y1) //源在目标左上，则从右向做，从下向上复制
    {
        if((X0 % 8) >= (X1 % 8)) //源数据需要左移
        {
            for(i = Len + 1; i >= 0; i --)
...
//pPoint1表示在目标中的
void Copy_Stretch_Rect(S_Show_Data *pSrc_Buf, INT8U Area_No, S_Point *pPoint0, INT16U X_Len, INT16U Y_Len, INT8U Stretch_X_Fac, INT8U Stretch_Y_Fac,\
                              S_Show_Data *pDst_Buf, S_Point *pPoint1)
{
    INT16U i,j;//,m,n;
    INT8U Re;
    ...
    for(i = 0; i < X_Len; i ++)
      for(j = 0; j < Y_Len; j ++)
      {
        Re = Get_Area_Point_Data(pSrc_Buf, Area_No, pPoint0->X + i, pPoint0->Y + j);
                ...
                Fill_Rect(pDst_Buf, Area_No, &P0, Stretch_X_Fac, Stretch_Y_Fac, 0);
                Set_Area_Point_Data(pDst_Buf, Area_No, P0.X, P0.Y, Re);
                              ...
                              S_Show_Data *pDst_Buf, S_Point *pPoint1)
{
    ...
    if(Stretch_X_Fac > 1)
    {
      for(i = 0; i < X_Len; i ++)
      {
          ...
          for(j = 0; j < Stretch_X_Fac; j ++)
          {
            P2.X = pPoint1->X + i*Stretch_X_Fac + j;
            P2.Y = pPoint1->Y;
            Copy_Line(pSrc_Buf, Area_No, &P0, &P1, pDst_Buf, &P2);
          }
      }
    }
    else
    {
        for(i = 0; i < Y_Len; i ++)
        {
            ...
            for(j = 0; j < Stretch_Y_Fac; j ++)
            {
                P2.Y = pPoint1->Y + i * Stretch_Y_Fac + j;
                P2.X = pPoint1->X;
              Copy_Line(pSrc_Buf, Area_No, &P0, &P1, pDst_Buf, &P2);
            }
        }
...
void Move_Left_Up_Cover0(INT8U Area_No)
{
    ...
    {
      Point0.X = Area_Width - Prog_Status.Area_Status[Area_No].Step;
      Point0.Y = Area_Height - Area_Height * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }
    else
    {
      Point0.Y = Area_Height - Prog_Status.Area_Status[Area_No].Step;
      Point0.X = Area_Width - Area_Width * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }

    Point1.X = Area_Width - 1;
    Point1.Y = Point0.Y;

    Point2.X = Point0.X;
    Point2.Y = Area_Height - 1;

    Copy_Line(&Show_Data_Bak, Area_No, &Point0, &Point1, &Show_Data, &Point0);
    Copy_Line(&Show_Data_Bak, Area_No, &Point0, &Point2, &Show_Data, &Point0);
...
void Move_Left_Down_Cover0(INT8U Area_No)
{
    ...
    {
      Point0.X = Area_Width - Prog_Status.Area_Status[Area_No].Step;
      Point0.Y = Area_Height * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }
    else
    {
      Point0.Y = Prog_Status.Area_Status[Area_No].Step;
      Point0.X = Area_Width - Area_Width * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }
...
void Move_Right_Up_Cover0(INT8U Area_No)
{
    ...
    if(Area_Width > Area_Height)
    {
      Point0.X = Prog_Status.Area_Status[Area_No].Step;
      Point0.Y = Area_Height - Area_Height * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }
    else
    {
      Point0.Y = Area_Height - Prog_Status.Area_Status[Area_No].Step;
      Point0.X = Area_Width * Prog_Status.Area_Status[Area_No].Step /Prog_Status.Area_Status[Area_No].Max_Step;
    }
...
//顺时针旋转风车
void Move_Spin_CW_Windmill(INT8U Area_No)
{
   //INT16U X,Y;
   //INT16U Arg;
   INT16U Area_Width, Area_Height;
   ...
   //--------------
   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Step - MOVE_STEP + (Area_Width + Area_Height) / 2, &Point[1]);
   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Step + (Area_Width + Area_Height) / 2, &Point[2]);
   //Copy_Line(&Show_Data_Bak, Area_No, &Point[0], &Point[1], &Show_Data, &Point[0]);
   Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);

   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Step - MOVE_STEP + (Area_Width + Area_Height) * 3 / 2, &Point[1]);
   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Step + (Area_Width + Area_Height) * 3 / 2, &Point[2]);
   //Copy_Line(&Show_Data_Bak, Area_No, &Point[0], &Point[1], &Show_Data, &Point[0]);
   Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);
}
...
//逆时针风车
void Move_Spin_CCW_Windmill(INT8U Area_No)
{
    //INT16U X;//,Y;
    //INT16U Arg;
    INT16U Area_Width, Area_Height;
    ...
    Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);
//---------------
    Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step + (Area_Width + Area_Height) / 2, &Point[1]);
    Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step + MOVE_STEP + (Area_Width + Area_Height) / 2, &Point[2]);
    Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);

    Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step + (Area_Width + Area_Height) * 3 / 2, &Point[1]);
    Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step + MOVE_STEP + (Area_Width + Area_Height) * 3 / 2, &Point[2]);
    Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);
...
//向上扇形合并
void Move_Up_Sector_Merge(INT8U Area_No)
{
   //INT16U X,Y;
   //INT16U Arg;
   INT16U Area_Width, Area_Height;
   ...
   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - (Prog_Status.Area_Status[Area_No].Step - MOVE_STEP), &Point[1]);
   Get_Spin_Step_Point(Area_Width, Area_Height, Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step, &Point[2]);
   //Copy_Line(&Show_Data_Bak, Area_No, &Point[0], &Point[1], &Show_Data, &Point[0]);
   Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);
...
//向下扇形合并
void Move_Down_Sector_Merge(INT8U Area_No)
{
   //INT16U X,Y;
   //INT16U Arg;
   INT16U Area_Width, Area_Height;
   ...
   Get_Spin_Step_Point(Area_Width, Area_Height, Area_Width + Area_Height - (Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step + MOVE_STEP), &Point[1]);
   Get_Spin_Step_Point(Area_Width, Area_Height, Area_Width + Area_Height - (Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step), &Point[2]);
   //Copy_Line(&Show_Data_Bak, Area_No, &Point[0], &Point[1], &Show_Data, &Point[0]);
   Copy_Filled_Polygon(&Show_Data_Bak, Area_No, &Point[0], 3, &Show_Data, &Point[0]);
...
//复制横向的压缩窗口
INT16U Copy_Compresssion_H_Rect(S_Show_Data *pSrc, INT8U Area_No, S_Point *pPoint0, INT16U X_Len, INT16U Y_Len, \
                              S_Show_Data *pDst, S_Point *pPoint1, INT16U Ratio, INT8U Direct)
{

    INT16U i;//, j;
    S_Point P0, P1,P2;
    INT16U TENSILE_STEP,Temp,Temp0,Temp1,X0;
    //INT16U Temp;
    ...
    if(Direct EQ 0)
    {
        Temp0 = (TENSILE_STEP - Ratio) / 2;
        Temp1 = (TENSILE_STEP + Ratio) / 2;

        for(i = 0; i < X_Len; i ++)
        {
            Temp = i %  TENSILE_STEP;

          if(Temp >= Temp0 && Temp <= Temp1)
          {
              P0.X = pPoint0->X + i;
              P0.Y = pPoint0->Y;
              ...
              P2.X = pPoint1->X + X0++;//(i / TENSILE_STEP)*Ratio + (i % TENSILE_STEP);
              P2.Y = pPoint1->Y;
    ...
    else
    {
        ...
        for(i = 0; i < X_Len; i ++)
        {
          ...
          if(Temp >= Temp0 && Temp <= Temp1)
          {
              ...
              P2.X = pPoint1->X + X_Len - 1- X0++;//((i / TENSILE_STEP)*Ratio + (i % TENSILE_STEP));
              P2.Y = pPoint1->Y;
              Copy_Line(pSrc, Area_No, &P0, &P1, pDst, &P2);
           }
         }
...
//复制纵向的压缩窗口
INT16U Copy_Compresssion_V_Rect(S_Show_Data *pSrc, INT8U Area_No, S_Point *pPoint0, INT16U X_Len, INT16U Y_Len, \
                              S_Show_Data *pDst, S_Point *pPoint1, INT16U Ratio, INT8U Direct)
{
    INT16U i,Y0;
    S_Point P0, P1,P2;
    INT16U TENSILE_STEP,Temp,Temp0,Temp1;
    ...
    for(i = 0; i < Y_Len; i ++)
        ...
        if(Direct EQ 0)
        {
            if(Temp >= Temp0 && Temp <= Temp1)
            {
              ...
              P2.X = pPoint1->X;
              P2.Y = pPoint1->Y + Y0++;//(i / TENSILE_STEP)*Ratio + (i % TENSILE_STEP);
        ...
        else
        {
            ...
            {
              P0.X = pPoint0->X;// + i;
              P0.Y = pPoint0->Y + Y_Len - 1 - i;

              P1.X = pPoint0->X + X_Len - 1;
              P1.Y = P0.Y;

              P2.X = pPoint1->X;
              P2.Y = pPoint1->Y + Y_Len - 1 - Y0++;//(i / TENSILE_STEP)*Ratio + (i % TENSILE_STEP);
...
void Move_Vertical_Tensile(INT8U Area_No)
{
    ...
    //P1.Y = P1.Y / 2;
    Len = Copy_Compresssion_V_Rect(&Show_Data_Bak, Area_No, &P0, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len / 2,\
                             &Show_Data, &P1, Step, 0);
    ...
    P1.X = 0;
    P1.Y = Prog_Para.Area[Area_No].Y_Len / 2 - Len; //Prog_Para.Area[Area_No].Y_Len - Step * Prog_Para.Area[Area_No].Y_Len / Prog_Status.Area_Status[Area_No].Max_Step;
    //P1.Y = P1.Y / 2;
...
void Move_Horizontal_Tensile(INT8U Area_No)
{
    ...
    //P1.Y = P1.Y / 2;
    Len = Copy_Compresssion_H_Rect(&Show_Data_Bak, Area_No, &P0, Prog_Para.Area[Area_No].X_Len / 2, Prog_Para.Area[Area_No].Y_Len,\
                             &Show_Data, &P1, Step, 0);

    P0.X = 0;
    P0.Y = 0;

    P1.X = Prog_Para.Area[Area_No].X_Len / 2 - Len;//Prog_Para.Area[Area_No].X_Len - Step * Prog_Para.Area[Area_No].X_Len / Prog_Status.Area_Status[Area_No].Max_Step;
    P1.Y = 0;
    //P1.X = P1.X / 2;
...
void Move_Left_Stretch(INT8U Area_No)
{
   ...
   else if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step*2 / 3)
   {
       ...
       Temp.X = Copy_Compresssion_H_Rect(&Show_Data_Bak, Area_No, &Temp, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len,\
                                &Show_Data, &Temp, Step, 0);

       Fill_Rect(&Show_Data, Area_No, &Temp, Prog_Para.Area[Area_No].X_Len - Temp.X, Prog_Para.Area[Area_No].Y_Len, 0);
...
void Move_Up_Stretch(INT8U Area_No)
{
   ...
   else if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step*2 / 3)
   {
       ...
       Temp.Y = 0;
       Step = (Prog_Status.Area_Status[Area_No].Max_Step - (Prog_Status.Area_Status[Area_No].Step*3 - Prog_Status.Area_Status[Area_No].Max_Step));
       Temp.Y = Copy_Compresssion_V_Rect(&Show_Data_Bak, Area_No, &Temp, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len,\
                                &Show_Data, &Temp, Step, 0);

       Fill_Rect(&Show_Data, Area_No, &Temp, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len - Temp.Y, 0);
...
//翻腾弹出
void Move_Jump_Stretch(INT8U Area_No)
{
  S_Point P0,P1,P2;
  ...
  else if(Step <= Max_Step * 2 / 3)
  {
    Step = Step - Max_Step / 3;
    Max_Step = Max_Step / 3;
  ...
  {
    Step = Step - Max_Step * 2 / 3;
    Max_Step = Max_Step / 3;

    Step_Flag = 2;
  }

  TENSILE_STEP = Get_TENSILE_STEP(Area_Height / 2);//Prog_Para.Area[Area_No].X_Len);

  if(Step_Flag EQ 0 || Step_Flag EQ 2)
  ...
  P1.X = Area_Width - 1;
  P2.X = 0;
  for(i = 0; i < Area_Height / 2; i ++)
  {
     Temp = i %  TENSILE_STEP;
     Temp0 = (TENSILE_STEP - Ratio) / 2;
     Temp1 = (TENSILE_STEP + Ratio) / 2;

     if(Temp >= Temp0 && Temp <= Temp1) //第一阶段
     {
         if(Step_Flag < 2)
         {
             P0.Y = Area_Height / 2 + i;
             P1.Y = Area_Height / 2 + i;
             P2.Y = Area_Height / 2 - Y0;
         }
         else
         {
             P0.Y = Area_Height / 2 + i;
  ...
  //Y0 = P2.Y;
  if(P2.Y < Area_Height / 2)
  {
      P0.X = 0;
      P0.Y = 0;
      Fill_Rect(&Show_Data, Area_No, &P0, Area_Width, P2.Y, 0);

      P0.X = 0;
      P0.Y = Area_Height - P2.Y;
      Fill_Rect(&Show_Data, Area_No, &P0, Area_Width, P2.Y, 0);
  }
  else
  {
      P0.X = 0;
      P0.Y = 0;
      Fill_Rect(&Show_Data, Area_No, &P0, Area_Width, Area_Height - P2.Y, 0);
...
void Move_Diamond(INT8U Area_No)
{
    ...
    {
        P[0].X = 0;//Step;//Prog_Para.Area[Area_No].X_Len / 2 - Step;
        P[0].Y = Step;//Prog_Para.Area[Area_No].Y_Len / 2;
    ...
    {
        P[0].X = Step - Prog_Para.Area[Area_No].Y_Len / 2;//Prog_Para.Area[Area_No].X_Len / 2 - Step;
        P[0].Y = Prog_Para.Area[Area_No].Y_Len / 2;//Prog_Para.Area[Area_No].Y_Len / 2 - (Step - Prog_Para.Area[Area_No].X_Len / 2);

        P[2].X = Prog_Para.Area[Area_No].X_Len - 1 - P[0].X;//Prog_Para.Area[Area_No].X_Len - 1;
        P[2].Y = P[0].Y;
    }

    if(Step < Prog_Para.Area[Area_No].X_Len / 2)
    {
        P[1].Y = 0;//Prog_Para.Area[Area_No].Y_Len / 2 - Step;
        P[1].X = Step;//Prog_Para.Area[Area_No].X_Len / 2;

        P[3].Y = Prog_Para.Area[Area_No].Y_Len - 1;//Prog_Para.Area[Area_No].Y_Len / 2 + Step;
        P[3].X = Prog_Para.Area[Area_No].X_Len - 1 - P[1].X;
    }
    else
    {
        P[1].X = Prog_Para.Area[Area_No].X_Len / 2;//Prog_Para.Area[Area_No].X_Len / 2 - (Step - Prog_Para.Area[Area_No].Y_Len / 2);
        P[1].Y = Step - Prog_Para.Area[Area_No].X_Len / 2;//Prog_Para.Area[Area_No].X_Len / 2 - Step;

        P[3].X = P[1].X;//Prog_Para.Area[Area_No].X_Len - 1 - P[1].X;
        P[3].Y = Prog_Para.Area[Area_No].Y_Len - 1 - P[1].Y;//Prog_Para.Area[Area_No].Y_Len - 1;
...
{
    S_Point Temp0,Temp1;
    INT16U Step;//Moved_Len, Last_Moved_Len;

       if(Prog_Status.Area_Status[Area_No].Step  <= Prog_Para.Area[Area_No].X_Len / STRETCH_RATIO)
       {
           //已复制的数据左移动
           Temp0.X = Prog_Para.Area[Area_No].X_Len - (Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO;
           Temp0.Y = 0;

           Temp1.X = Prog_Para.Area[Area_No].X_Len - Prog_Status.Area_Status[Area_No].Step*STRETCH_RATIO;// + Step +
           Temp1.Y = 0;

           Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len - Temp0.X, Prog_Para.Area[Area_No].Y_Len, &Show_Data, &Temp1, 0);
       }
       else
       {
         Step = Prog_Status.Area_Status[Area_No].Step * STRETCH_RATIO - Prog_Para.Area[Area_No].X_Len;

         Temp0.X = (Step > STRETCH_RATIO)?(Step - STRETCH_RATIO):0;

         Temp0.Y = 0;
         Copy_Filled_Rect(&Show_Data_Bak, Area_No, &Temp0, STRETCH_RATIO, Prog_Para.Area[Area_No].Y_Len, &Show_Data, &Temp0, 0);

         Temp0.X = Step + STRETCH_RATIO;
         Temp0.Y = 0;
       ...
       if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
       {
         Temp0.X = (Prog_Status.Area_Status[Area_No].Step - 1);
         Temp0.Y = 0;
         ...
         Temp1.X = Prog_Para.Area[Area_No].X_Len - STRETCH_RATIO;// + Step +
         Temp1.Y = 0;

         Copy_Filled_Stretch_Rect(&Show_Data_Bak, Area_No, &Temp0, 1, Prog_Para.Area[Area_No].Y_Len, STRETCH_RATIO, 1, &Show_Data, &Temp1);
       }
}
...
void Move_Right_Compress_0(INT8U Area_No)
{
    ...
    INT16U Step;//Moved_Len, Last_Moved_Len;
       ...
       if(Prog_Status.Area_Status[Area_No].Step  <= Prog_Para.Area[Area_No].X_Len / STRETCH_RATIO)
       {
           Temp0.X = 0;//(Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO;
           Temp0.Y = 0;

           Temp1.X = STRETCH_RATIO;// + Step +
           Temp1.Y = 0;

           Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, (Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO, Prog_Para.Area[Area_No].Y_Len, &Show_Data, &Temp1, 0);
       }
       else
       {
         Step = Prog_Status.Area_Status[Area_No].Step * STRETCH_RATIO - Prog_Para.Area[Area_No].X_Len;

         Temp0.X = Prog_Para.Area[Area_No].X_Len - Step;//((Step > STRETCH_RATIO)?(Step - STRETCH_RATIO):0);
         Temp0.Y = 0;
         ...
         Temp1.X = STRETCH_RATIO;
         Temp1.Y = 0;
         Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len - STRETCH_RATIO - Step, Prog_Para.Area[Area_No].Y_Len, &Show_Data, &Temp1, 0);
       }
     }
    ...
    if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
    {
     Temp0.X = Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step;
     Temp0.Y = 0;

     //Moved_Len = Prog_Status.Area_Status[Area_No].Step*STRETCH_RATIO * Prog_Para.Area[Area_No].X_Len / Prog_Status.Area_Status[Area_No].Max_Step;
     Temp1.X = 0;//Prog_Para.Area[Area_No].X_Len - STRETCH_RATIO;// + Step +
     Temp1.Y = 0;
...
void Move_Up_Compress_0(INT8U Area_No)
{
    ...
    INT16U Step;//Moved_Len, Last_Moved_Len;
       ...
       if(Prog_Status.Area_Status[Area_No].Step  <= Prog_Para.Area[Area_No].Y_Len / STRETCH_RATIO)
       {
           //已复制的数据左移动
           Temp0.Y = Prog_Para.Area[Area_No].Y_Len - (Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO;
           Temp0.X = 0;

           Temp1.Y = Prog_Para.Area[Area_No].Y_Len - Prog_Status.Area_Status[Area_No].Step*STRETCH_RATIO;// + Step +
           Temp1.X = 0;

           Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len - Temp0.Y,  &Show_Data, &Temp1, 0);
       }
       else
       {
         Step = Prog_Status.Area_Status[Area_No].Step * STRETCH_RATIO - Prog_Para.Area[Area_No].Y_Len;

         Temp0.Y = (Step > STRETCH_RATIO)?(Step - STRETCH_RATIO):0;

         Temp0.X = 0;
         Copy_Filled_Rect(&Show_Data_Bak, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, STRETCH_RATIO,  &Show_Data, &Temp0, 0);

         Temp0.Y = Step + STRETCH_RATIO;
         Temp0.X = 0;
       ...
       if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
       {
         Temp0.Y = (Prog_Status.Area_Status[Area_No].Step - 1);
         Temp0.X = 0;

         //Moved_Len = Prog_Status.Area_Status[Area_No].Step*STRETCH_RATIO * Prog_Para.Area[Area_No].X_Len / Prog_Status.Area_Status[Area_No].Max_Step;
         Temp1.Y = Prog_Para.Area[Area_No].Y_Len - STRETCH_RATIO;// + Step +
         Temp1.X = 0;

         Copy_Filled_Stretch_Rect(&Show_Data_Bak, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, 1, 1, STRETCH_RATIO, &Show_Data, &Temp1);
       }
...
void Move_Down_Compress_0(INT8U Area_No)
{
    ...
    INT16U Step;//Moved_Len, Last_Moved_Len;
       ...
       if(Prog_Status.Area_Status[Area_No].Step  <= Prog_Para.Area[Area_No].Y_Len / STRETCH_RATIO)
       {
           Temp0.Y = 0;//(Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO;
           Temp0.X = 0;

           Temp1.Y = STRETCH_RATIO;// + Step +
           Temp1.X = 0;

           Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, (Prog_Status.Area_Status[Area_No].Step - 1) * STRETCH_RATIO, &Show_Data, &Temp1, 0);
       }
       else
       {
         Step = Prog_Status.Area_Status[Area_No].Step * STRETCH_RATIO - Prog_Para.Area[Area_No].Y_Len;

         Temp0.Y = Prog_Para.Area[Area_No].Y_Len - Step;//((Step > STRETCH_RATIO)?(Step - STRETCH_RATIO):0);
         Temp0.X = 0;
         ...
         Temp1.Y = STRETCH_RATIO;
         Temp1.X = 0;
         Copy_Filled_Rect(&Show_Data, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, Prog_Para.Area[Area_No].Y_Len - STRETCH_RATIO - Step,  &Show_Data, &Temp1, 0);
       }
     }
    ...
    if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
    {
     Temp0.Y = Prog_Status.Area_Status[Area_No].Max_Step - Prog_Status.Area_Status[Area_No].Step;
     Temp0.X = 0;

     //Moved_Len = Prog_Status.Area_Status[Area_No].Step*STRETCH_RATIO * Prog_Para.Area[Area_No].X_Len / Prog_Status.Area_Status[Area_No].Max_Step;
     Temp1.Y = 0;//Prog_Para.Area[Area_No].X_Len - STRETCH_RATIO;// + Step +
     Temp1.X = 0;
...
//左分散压入
void Move_Left_Compress_1(INT8U Area_No)
{
    S_Point Temp0,Temp1;
    INT16U Step;//Moved_Len, Last_Moved_Len;
       ...
       if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
       {
         ...
         Copy_Stretch_Rect(&Show_Data_Bak, Area_No, &Temp0, 1, Prog_Para.Area[Area_No].Y_Len, STRETCH_RATIO, 1, &Show_Data, &Temp1);
       }
}

//右分散压入
void Move_Right_Compress_1(INT8U Area_No)
{
    S_Point Temp0,Temp1;
    INT16U Step;//Moved_Len, Last_Moved_Len;
...
//上分散压入
void Move_Up_Compress_1(INT8U Area_No)
{
    S_Point Temp0,Temp1;
    INT16U Step;//Moved_Len, Last_Moved_Len;
       ...
       if(Prog_Status.Area_Status[Area_No].Step < Prog_Status.Area_Status[Area_No].Max_Step)
       {
         ...
         Copy_Stretch_Rect(&Show_Data_Bak, Area_No, &Temp0, Prog_Para.Area[Area_No].X_Len, 1, 1, STRETCH_RATIO, &Show_Data, &Temp1);
       }
}

//下分散压入
void Move_Down_Compress_1(INT8U Area_No)
{
    S_Point Temp0,Temp1;
    INT16U Step;//Moved_Len, Last_Moved_Len;
...
//矩形展开
void Move_Rect_Open(INT8U Area_No)
{
  S_Point P[4];
  INT16U Area_Width,Area_Height;
  ...
  P[0].X = (Area_Width + 1) / 2 - (Area_Width + 1) / 2 * Prog_Status.Area_Status[Area_No].Step/ Prog_Status.Area_Status[Area_No].Max_Step;
  P[0].Y = (Area_Height + 1) / 2 - (Area_Height + 1) / 2 * Prog_Status.Area_Status[Area_No].Step/ Prog_Status.Area_Status[Area_No].Max_Step;

  P[1].X = Area_Width - 1 - P[0].X;
  P[1].Y = P[0].Y;

  P[2].X = P[1].X;
  P[2].Y = Area_Height - 1 - P[1].Y;

  P[3].X = P[0].X;
  P[3].Y = Area_Height - 1 - P[0].Y;
...
//矩形收缩
void Move_Rect_Close(INT8U Area_No)
{
    S_Point P[4];
    INT16U Area_Width,Area_Height;
    ...
    P[0].X = (Area_Width + 1) / 2 * Prog_Status.Area_Status[Area_No].Step/ Prog_Status.Area_Status[Area_No].Max_Step;
    P[0].Y = (Area_Height + 1) / 2 * Prog_Status.Area_Status[Area_No].Step/ Prog_Status.Area_Status[Area_No].Max_Step;
...
//左右交叉打开
void Move_Left_Right_Cross_Open(INT8U Area_No)
{
    S_Point P1,P2;
    INT16U Area_Width,Area_Height;
    ...
    P1.X = Prog_Status.Area_Status[Area_No].Step * 2;
    P1.Y = 0;

    P2.X = P1.X;
    P2.Y = Area_Height - 1;

    Copy_Line(&Show_Data_Bak, Area_No, &P1, &P2, &Show_Data, &P1);

    P1.X = Area_Width / 2 * 2 + 1 - Prog_Status.Area_Status[Area_No].Step * 2;
    P1.Y = 0;
...
//上下交叉拉姆
void Move_Up_Down_Cross_Open(INT8U Area_No)
{
    S_Point P1,P2;
    INT16U Area_Width,Area_Height;
    ...
    P1.Y = Prog_Status.Area_Status[Area_No].Step * 2;
    P1.X = 0;
    ...
    P1.Y = Area_Height / 2 * 2 + 1 - Prog_Status.Area_Status[Area_No].Step * 2;
    P1.X = 0;
...
//左右中间溢出
void Move_LR_Mid_Overflow(INT8U Area_No)
{
  S_Point P[2];
  ...
  Copy_Filled_Rect(&Show_Data_Bak, Area_No, &P[0], Len, Area_Height - 1, &Show_Data, &P[1], 0);
...
//上下中间溢出
void Move_UD_Mid_Overflow(INT8U Area_No)
{
    S_Point P[2];
    INT16U Step, Max_Step, Len, Area_Width, Area_Height;
    ...
    Copy_Filled_Rect(&Show_Data_Bak, Area_No, &P[0], Area_Width - 1, Len, &Show_Data, &P[1], 0);
